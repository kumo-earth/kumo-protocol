...
describe("EthersKumoRedemption", async () => {
    ...
    describe("Redemption Bootstrap phase Multi Asset", () => {
    ...
    describe("Redemption Multi Asset Tests", () => {
        before(async function () {
        ...
        it(`should redeem some KUSD after the bootstrap phase`, async () => {
            const kusdBalanceBefore = (await kumo.getKUSDBalance())
            const newTroves = mockAssetContracts.map(async mockAssetContract => {
                ...
                const kusdBalanceBefore = (await kumo.getKUSDBalance()).sub(someKUSD)
                const { rawReceipt, details } = await waitForSuccess(kumo.send.redeemKUSD(mockAssetAddress, someKUSD));
                return { expectedDetails, details, mockAssetAddress, mockAsset }
            })

            for await (const trove of newTroves) {
                const { expectedDetails, details, mockAssetAddress, mockAsset } = trove
                ...
                expect(`${await kumo.getKUSDBalance()}`).to.equal(`${kusdBalanceBefore.sub(someKUSD.mul(newTroves.length))}`);
        ...
        it(`should claim the collateral surplus after redemption`, async () => {
            const newTroves = mockAssetContracts.map(async mockAssetContract => {
                ...
                return { asset1balanceBefore1, asset1balanceBefore2, surplus1, surplus2, mockAssetAddress, mockAsset }

            })
            for await (const trove of newTroves) {
                const { asset1balanceBefore1, asset1balanceBefore2, surplus1, surplus2, mockAssetAddress, mockAsset } = trove
                expect(`${await otherKumos[0].getCollateralSurplusBalance(mockAssetAddress)}`).to.equal("0");
                expect(`${await otherKumos[1].getCollateralSurplusBalance(mockAssetAddress)}`).to.equal("0");
                expect(`${await otherKumos[2].getCollateralSurplusBalance(mockAssetAddress)}`).to.equal("0");
