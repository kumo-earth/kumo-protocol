 ...
 *
 * @remarks
 * Returned by the {@link ReadableKumo.getFees | getFees()} function.
 *
 * @public
 */
export class Fees {
   ...
   * the `when` parameter.
   *
   * To calculate the borrowing fee in KUSD, multiply the borrowed KUSD amount by the borrowing rate.
   *
   * @example
   * ```typescript
   ...
   *
   * @param redeemedFractionOfSupply - The amount of KUSD being redeemed divided by the total supply.
   ...
   * precise, it depends on the fraction of the redeemed amount compared to the total KUSD supply,
   * which must be passed as a parameter.
   *
   * To calculate the redemption fee in KUSD, multiply the redeemed KUSD amount with the redemption
   * rate.
   *
   * @example
   * ```typescript
