...
KUSD_GAS_COMPENSATION = 200.0
MIN_NET_DEBT = 1800.0
MAX_FEE = Wei(1e18)

"""# Ether price (exogenous)

Ether is the collateral for KUSD. The ether price $P_t^e$ follows 
...
"""# KUMO price
In the first month, the price of KUMO follows

> $P_t^q = P_{t-1}^q (1+\zeta_t^q)(1+\sigma_t^q)$. 

Note that $\zeta_t^q \sim N(0,$ sd_KUMO) represents KUMO price shock and $\sigma_t^q$ the drift. Here, $\sigma_t^q =$ drift_KUMO, so that the expected KUMO price increases from price_KUMO_initial to the following at the end of the first month:
> $E(P_{720}^q) = $price_KUMO_initial$ \cdot (1+$ drift_KUMO$)^{720}$

The KUMO price from the second month on is endogenously determined.
"""

#KUMO price & airdrop
price_KUMO_initial = 0.4
price_KUMO = [price_KUMO_initial]
sd_KUMO=0.005
drift_KUMO = 0.0035
supply_KUMO=[0]
KUMO_total_supply=100000000

"""**KUMO Endogenous Price**

The staked KUMO pool earning consists of the issuance fee revenue and redemption fee revenue
> $R_t^q = R_t^i + R_t^r.$
...
> $$r_t=\frac{KUMO Market Cap}{Annualized Earning}=\frac{MC_t}{E_t}$$

> $MC_t=P_t^q \cdot$ KUMO_total_supply

Therefore, the KUMO price dynamics is determined
> $$P_t^q=discount \cdot \frac{r^{PE}}{KUMO\_total\_supply}E_t$$

Interpretation: The denominator implies that with more KUMO tokens issued, KUMO price decreases. However, the depreciation effect can be counteracted by the growth of the earning.
...
$R_{t-1}^s$ is the return in the stability pool, which consists of liquidation gain and airdrop KUMO gain.
...
> (collateral in ether, debt in KUSD, collateral ratio target, rational inattention, collateral ratio)
...
$P_{t}^{l}$ is the price of KUSD.

$N_t^o$ is rounded to an integer.

---

The amount of KUSD tokens generated by a new trove is
...
#sensitivity to KUSD price & issuance fee
alpha = 0.3
...
- $R_t^a=P_{t}^q\hat{Q}_t^q$ is the airdrop gain, $\hat{Q}_t^q=1000$ denotes the amount of KUMO token airdropped to the stability pool providers
- $D_{t}^{s}$ is the total amount of KUSD deposited in the Stability Pool (see below)
...
"""KUMO Price - First Month"""

#KUMO price
for i in range(1, month):
    random.seed(2+13*i)
    shock_KUMO = random.normalvariate(0,sd_KUMO)  
    price_KUMO.append(price_KUMO[i-1]*(1+shock_KUMO)*(1+drift_KUMO))
...
def pending_liquidations(contracts, price_ether_current):
    ...
    stability_pool_balance = contracts.stabilityPool.getTotalKUSDDeposits()
    trove = last_trove
...
# F = 0.5 ** (1/8760)
# F = 0.99992087674
def quantity_KUMO_airdrop(index):
    F = 0.99992087674
    if index <= 0:
        return 0
    return 32e6 * (F ** (index-1) - F ** index)

def liquidate_troves(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, price_KUSD, price_KUMO_current, data, index):
    if len(active_accounts) == 0:
        return [0, 0]

    stability_pool_previous = contracts.stabilityPool.getTotalKUSDDeposits() / 1e18
    ...
    while pending_liquidations(contracts, price_ether_current):
        try:
            ...
            for i in range(NUM_LIQUIDATIONS):
                ...
                ICR = contracts.troveManager.getCurrentICR(trove, Wei(price_ether_current * 1e18))
                print(f"ICR: {ICR}")
    stability_pool_current = contracts.stabilityPool.getTotalKUSDDeposits() / 1e18
    ...
    liquidation_gain = ether_liquidated * price_ether_current - debt_liquidated * price_KUSD
    airdrop_gain = price_KUMO_current * quantity_KUMO_airdrop(index)

    data['liquidation_gain'][index] = liquidation_gain
    data['airdrop_gain'][index] = airdrop_gain

    return_stability = calculate_stability_return(contracts, price_KUSD, data, index)

    return [ether_liquidated, return_stability]

def calculate_stability_return(contracts, price_KUSD, data, index):
    stability_pool_previous = contracts.stabilityPool.getTotalKUSDDeposits() / 1e18
    if index == 0:
        ...
        return_stability = (year/index) * \
            (sum(data['liquidation_gain'][0:index]) +
             ...
             ) / (price_KUSD * stability_pool_previous)
    else:
...
def close_troves(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, price_KUSD, index):
    if len(active_accounts) == 0:
        return [0]
    ...
    if index <= 240:
        ...
        number_closetroves = max(0, n_steady * (1+shock_closetroves)) + beta*(1-price_KUSD)*n_troves
    ...
    for i in range(0, len(drops)):
        ...
        coll = amounts['coll']
        debt = amounts['debt']
        pending = get_kusd_to_repay(accounts, contracts, active_accounts, inactive_accounts, account, debt)
        if pending == 0:
...
def transfer_from_to(contracts, from_account, to_account, amount):
    balance = contracts.kusdToken.balanceOf(from_account)
    transfer_amount = min(balance, amount)
    if transfer_amount == 0:
        return amount
    if from_account == to_account:
        return amount
    contracts.kusdToken.transfer(to_account, transfer_amount, { 'from': from_account })
    pending = amount - transfer_amount

    return pending

def get_kusd_to_repay(accounts, contracts, active_accounts, inactive_accounts, account, debt):
    kusdBalance = contracts.kusdToken.balanceOf(account)
    if debt > kusdBalance:
        pending = debt - kusdBalance
        ...
        # try with active accounts, which are more likely to hold KUSD
        ...
        if pending > 0:
            print(f"\n ***Error: not enough KUSD to repay! {debt / 1e18} KUSD for {account}")
...
def adjust_troves(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, index):
    ...
    for i, working_trove in enumerate(active_accounts):
        ...
        if p >= ratio:
            ...
            if check < -1:
                ...
                pending = get_kusd_to_repay(accounts, contracts, active_accounts, inactive_accounts, account, repay_amount)
                if pending == 0:
                    contracts.borrowerOperations.repayKUSD(repay_amount, hints[0], hints[1], { 'from': account })
                ...
                if isNewTCRAboveCCR(contracts, 0, False, withdraw_amount_wei, True, floatToWei(price_ether_current)):
                    contracts.borrowerOperations.withdrawKUSD(MAX_FEE, withdraw_amount_wei, hints[0], hints[1], { 'from': account })
                    rate_issuance = contracts.troveManager.getBorrowingRateWithDecay() / 1e18
                    issuance_KUSD_adjust = issuance_KUSD_adjust + rate_issuance * withdraw_amount
    ...
    return [coll_added_float, issuance_KUSD_adjust]
...
def open_trove(accounts, contracts, active_accounts, inactive_accounts, supply_trove, quantity_ether, CR_ratio, rational_inattention, price_ether_current):
    ...
    debtChange = floatToWei(supply_trove) + KUSD_GAS_COMPENSATION
    kusd = get_kusd_amount_from_net_debt(contracts, floatToWei(supply_trove))
    if isNewTCRAboveCCR(contracts, coll, True, debtChange, True, floatToWei(price_ether_current)):
        contracts.borrowerOperations.openTrove(MAX_FEE, kusd, hints[0], hints[1],
...
def open_troves(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, price_KUSD, index):
    random.seed(2019*index)
    shock_opentroves = random.normalvariate(0,sd_opentroves)
    n_troves = len(active_accounts)
    rate_issuance = contracts.troveManager.getBorrowingRateWithDecay() / 1e18
    coll_added = 0
    issuance_KUSD_open = 0

    if index <= 0:
        number_opentroves = initial_open
    elif price_KUSD <= 1 + rate_issuance:
        number_opentroves = max(0, n_steady * (1+shock_opentroves))
    else:
        number_opentroves = max(0, n_steady * (1+shock_opentroves)) + \
                        alpha * (price_KUSD - rate_issuance - 1) * n_troves
    ...
    for i in range(0, number_opentroves):
        ...
        issuance_KUSD_open = issuance_KUSD_open + rate_issuance * supply_trove
    ...
    return [coll_added, issuance_KUSD_open]
...
def stability_update(accounts, contracts, active_accounts, return_stability, index):
    supply = contracts.kusdToken.totalSupply() / 1e18
    stability_pool_previous = contracts.stabilityPool.getTotalKUSDDeposits() / 1e18
    ...
    if stability_pool > stability_pool_previous:
        ...
        while remaining > 0 and i < len(active_accounts):
          ...
          balance = contracts.kusdToken.balanceOf(account) / 1e18
        ...
        current_deposit = contracts.stabilityPool.getCompoundedKUSDDeposit(accounts[0])
...
"""KUSD Price, liquidity pool, and redemption

**Price Determination**

---
With the supply and demand of KUSD tokens defined above, the price of KUSD at the current period is given by the following equilibrium condition:
> $$S_t = D_t^s + D_t^l = D_t^s + D_{t-1}^l (1+\zeta_t^l)(1+\sigma_t^l) (\frac{P_t^l}{P_{t-1}^l})^\delta$$

where $S$ is the total supply of KUSD.

Solving this equation gives that:
> $$P_t^l = P_{t-1}^l (\frac{S_t-D_t^s}{D_{t-1}^l(1+\zeta_t^l)(1+\sigma_t^l)})^{1/\delta}$$
"""

def calculate_price(price_KUSD, liquidity_pool, liquidity_pool_next):
  # liquidity_pool = supply - stability_pool
  # liquidity_pool_next = liquidity_pool_previous * drift_liquidity * (1+shock_liquidity)
  price_KUSD_current= price_KUSD * (liquidity_pool / liquidity_pool_next) ** (1/delta)

  return price_KUSD_current

""" **Stabilizers**

There are two stabilizers to attenuate KUSD price deviation from its target range.
...
The amount of $Q_t^d(c)$ is expected to bring the KUSD price back to $1.1+f_t^i$. This means that
...
If $P_t^l < 1-f_t^r$, a fraction $\chi_t$ of KUSD in the liquidity pool is used for redemption
> $$D_t^r = \chi_t D_t^l,$$
...
Note that unlike stabilizer 1, stabilizer 2 has impact of KUSD price in
...
def redeem_trove(accounts, contracts, i, price_ether_current):
    kusd_balance = contracts.kusdToken.balanceOf(accounts[i])
    [firstRedemptionHint, partialRedemptionHintNICR, truncatedKUSDamount] = contracts.hintHelpers.getRedemptionHints(kusd_balance, price_ether_current, 70)
    if truncatedKUSDamount == Wei(0):
        return None
    ...
    try:
        ...
        print(f"KUSD Token:    {contracts.kusdToken.address}")
        print(f"i: {i}")
        print(f"account: {accounts[i]}")
        print(f"KUSD bal: {kusd_balance / 1e18}")
        print(f"truncated: {truncatedKUSDamount / 1e18}")
        ...
        print(f"amount: {truncatedKUSDamount}")
...
def price_stabilizer(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, price_KUSD, index):

    stability_pool = contracts.stabilityPool.getTotalKUSDDeposits() / 1e18
    redemption_pool = 0
    redemption_fee = 0
    issuance_KUSD_stabilizer = 0
    ...
    #Calculating Price
    price_KUSD_current = calculate_price(price_KUSD, liquidity_pool, liquidity_pool_next)
    rate_issuance = contracts.troveManager.getBorrowingRateWithDecay() / 1e18
    rate_redemption = contracts.troveManager.getRedemptionRateWithDecay() / 1e18

    #Stabilizer
    #Ceiling Arbitrageurs
    if price_KUSD_current > 1.1 + rate_issuance:
        supply_wanted = stability_pool + \
                        liquidity_pool_next * \
                        ((1.1+rate_issuance) / price_KUSD)**delta
        ...
        issuance_KUSD_stabilizer = rate_issuance * supply_trove
        if open_trove(accounts, contracts, active_accounts, inactive_accounts, supply_trove, quantity_ether, CR_ratio, rational_inattention):
            price_KUSD_current = 1.1 + rate_issuance
            liquidity_pool = supply_wanted - stability_pool


    #Floor Arbitrageurs
    if price_KUSD_current < 1 - rate_redemption:
        ...
        supply_target = stability_pool + \
                        liquidity_pool_next * \
                        ((1-rate_redemption) / price_KUSD)**delta
        ...
        if supply_diff < redemption_ratio * liquidity_pool:
            ...
            price_KUSD_current = 1 - rate_redemption
        else:
    ...
    return [price_KUSD_current, redemption_pool, redemption_fee, issuance_KUSD_stabilizer]

"""# KUMO Market"""

def KUMO_market(index, data):
    #quantity_KUMO = (KUMO_total_supply/3)*(1-0.5**(index/period))
    np.random.seed(2+3*index)
    if index <= month:
        price_KUMO_current = price_KUMO[index-1]
        ...
        price_KUMO_current = discount * PE_ratio * annualized_earning / KUMO_total_supply

    #MC_KUMO_current = price_KUMO_current * quantity_KUMO

    return [price_KUMO_current, annualized_earning]
