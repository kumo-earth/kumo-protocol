...
def get_kusd_amount_from_net_debt(contracts, net_debt):
...
def logGlobalState(contracts):
    ...
    activePoolDebt = contracts.activePool.getKUSDDebt()
    defaultPoolColl = contracts.defaultPool.getETH()
    defaultPoolDebt = contracts.defaultPool.getKUSDDebt()
    ...
    SP_KUSD = contracts.stabilityPool.getTotalKUSDDeposits().to("ether")
    SP_ETH = contracts.stabilityPool.getETH().to("ether")
    print('SP KUSD         ', SP_KUSD)
    print('SP ETH          ', SP_ETH)
    ...
    return [ETH_price, num_troves, total_coll, total_debt, TCR, recovery_mode, last_ICR, SP_KUSD, SP_ETH]
