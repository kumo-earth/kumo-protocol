...
contract('TroveManager', async accounts => {
  ...
  it("A given trove's stake decline is negligible with adjustments and tiny liquidations", async () => {
    ...
    await borrowerOperations.openTrove(th._100pct, await getOpenTroveKUSDAmount(dec(1, 31)), erc20.address, erc20.address, { from: A, value: dec(2, 29) })
    
    // Make 5 large troves B, C, D, E, F at ~10% total collateral
    await borrowerOperations.openTrove(th._100pct, await getOpenTroveKUSDAmount(dec(2, 30)), erc20.address, erc20.address, { from: B, value: dec(4, 28) })
    await borrowerOperations.openTrove(th._100pct, await getOpenTroveKUSDAmount(dec(2, 30)), erc20.address, erc20.address, { from: C, value: dec(4, 28) })
    await borrowerOperations.openTrove(th._100pct, await getOpenTroveKUSDAmount(dec(2, 30)), erc20.address, erc20.address, { from: D, value: dec(4, 28) })
    await borrowerOperations.openTrove(th._100pct, await getOpenTroveKUSDAmount(dec(2, 30)), erc20.address, erc20.address, { from: E, value: dec(4, 28) })
    await borrowerOperations.openTrove(th._100pct, await getOpenTroveKUSDAmount(dec(2, 30)), erc20.address, erc20.address, { from: F, value: dec(4, 28) })
    ...
    for (account of tinyTroves) {
      await borrowerOperations.openTrove(th._100pct, await getOpenTroveKUSDAmount(dec(1, 22)), erc20.address, erc20.address, { from: account, value: dec(2, 20) })
    }
    ...
    await borrowerOperations.adjustTrove(th._100pct, 0, 1, false, erc20.address, erc20.address, {from: B})  // B repays 1 wei
    ...
    for (let [idx, trove] of tinyTroves.entries()) {
      ...
      await borrowerOperations.adjustTrove(th._100pct, 0, 1, false, erc20.address, erc20.address, {from: B})  // A repays 1 wei
