...
contract('CollSurplusPool', async accounts => {
  ...
  it("CollSurplusPool: claimColl(): Reverts if caller is not Borrower Operations", async () => {
    await th.assertRevert(collSurplusPool.claimColl(assetAddress1, A, { from: A }), 'CollSurplusPool: Caller is not Borrower Operations')
  })
  ...
  it.skip("CollSurplusPool: claimColl(): Reverts if owner cannot receive ETH surplus", async () => {
    ...
    const openTroveData = th.getTransactionData('openTrove(address,uint256,uint256,uint256,address,address)', [assetAddress1, 0, '0xde0b6b3a7640000', web3.utils.toHex(B_kusdAmount), B, B])
    await nonPayable.forward(borrowerOperations.address, openTroveData, { value: B_coll })
    await openTrove({ extraKUSDAmount: B_netDebt, extraParams: { from: A, value: dec(3000, 'ether') } })
    ...
    await th.assertRevert(nonPayable.forward(borrowerOperations.address, claimCollateralData), 'CollSurplusPool: sending Asset failed')
  })
  ...
  it('CollSurplusPool: accountSurplus: reverts if caller is not Trove Manager', async () => {
    await th.assertRevert(collSurplusPool.accountSurplus(assetAddress1, A, 1), 'CollSurplusPool: Caller is not TroveManager')
  })
})
