...
contract('TroveManager - in Recovery Mode', async accounts => {
  ...
  it("liquidate(), with ICR < 100%: removes stake and updates totalStakes", async () => {
    ...
    const bob_Stake_Before = (await troveManager.Troves(bob, assetAddress1))[TroveData.coll]
    ...
    const bob_Stake_After = (await troveManager.Troves(bob, assetAddress1))[TroveData.coll]
  ...
  it("liquidate(), with ICR < 100%: closes the Trove and removes it from the Trove array", async () => {
    ...
    const bob_TroveStatus_Before = (await troveManager.Troves(bob, assetAddress1))[TroveData.status]
    ...
    const bob_TroveStatus_After = (await troveManager.Troves(bob, assetAddress1))[TroveData.status]
  ...
  it("liquidate(), with ICR < 100%: only redistributes to active Troves - no offset to Stability Pool", async () => {
    ...
    await stabilityPool.provideToSP(spDeposit, erc20.address, { from: alice })
  ...
  it("liquidate(), with ICR > 110%, trove has lowest ICR, and StabilityPool is empty: does nothing", async () => {
    ...
    const bob_TroveStatus = bob_Trove[TroveData.status].toString()
  ...
  it("liquidate(), with  110% < ICR < TCR, and StabilityPool KUSD > debt to liquidate: removes stake and updates totalStakes", async () => {
    ...
    // Alice deposits KUSD in the Stability Pool
    await stabilityPool.provideToSP(B_totalDebt.add(toBN(1)), erc20.address, { from: alice })
  ...
  it("liquidate(), with ICR > 110%, and StabilityPool KUSD < liquidated debt: Trove remains active", async () => {
    ...
    await stabilityPool.provideToSP('1490000000000000000000', erc20.address, { from: alice })
  ...
  it("liquidate(), with ICR > 110%, and StabilityPool KUSD < liquidated debt: Trove remains in TroveOwners array", async () => {
    ...
    await stabilityPool.provideToSP(dec(100, 18), erc20.address, { from: alice })
    ...
    const idxOnStruct = (await troveManager.Troves(bob, assetAddress1))[TroveData.arrayIndex].toString()
  ...
  it("liquidate(), with ICR > 110%, and StabilityPool KUSD < liquidated debt: nothing happens", async () => {
    ...
    */
    const bob_Trove = await troveManager.Troves(bob, assetAddress1)
    const bob_DebtAfter = bob_Trove[TroveData.debt].toString()
    const bob_CollAfter = bob_Trove[TroveData.coll].toString()
    const bob_StakeAfter = bob_Trove[TroveData.stake].toString()
  ...
  it("liquidate(), with ICR > 110%, and StabilityPool KUSD < liquidated debt: ICR of non liquidated trove does not change", async () => {
    ...
    const bob_Coll_Before = (await troveManager.Troves(bob, assetAddress1))[TroveData.coll]
    const bob_Debt_Before = (await troveManager.Troves(bob, assetAddress1))[TroveData.debt]
    ...
    const bob_Coll_After = (await troveManager.Troves(bob, assetAddress1))[TroveData.coll]
    const bob_Debt_After = (await troveManager.Troves(bob, assetAddress1))[TroveData.debt]
    ...
    await stabilityPool.provideToSP(dec(50, 18), erc20.address, { from: alice })

    assert.isTrue(await th.checkRecoveryMode(contracts, assetAddress1))

    const carol_Coll_Before = (await troveManager.Troves(carol, assetAddress1))[TroveData.coll]
    const carol_Debt_Before = (await troveManager.Troves(carol, assetAddress1))[TroveData.debt]
    ...
    const carol_Debt_After = (await troveManager.Troves(carol, assetAddress1))[TroveData.debt]
  ...
  it("liquidate(): Liquidates undercollateralized trove if there are two troves in the system", async () => {
    ...
    await stabilityPool.provideToSP(dec(10, 18), erc20.address, { from: alice })
  ...
  it("liquidate(): does not affect the SP deposit or ETH gain when called on an SP depositor's address that has no trove", async () => {
    ...
    await stabilityPool.provideToSP(spDeposit, erc20.address, { from: dennis })
  ...
  it("liquidate(), with 110% < ICR < TCR, can claim collateral, re-open, be reedemed and claim again", async () => {
    ...
    // Alice deposits KUSD in the Stability Pool
    await stabilityPool.provideToSP(B_totalDebt, erc20.address, { from: alice })
  ...
  it("liquidateTroves(): With all ICRs > 110%, Liquidates Troves until system leaves recovery mode", async () => {
    ...
    await stabilityPool.provideToSP(liquidationAmount, erc20.address, { from: alice })
    ...
    */
    ...
    assert.equal(alice_Trove[TroveData.status], 1)
    assert.equal(bob_Trove[TroveData.status], 1)
    assert.equal(carol_Trove[TroveData.status], 1)
    assert.equal(dennis_Trove[TroveData.status], 1)
    assert.isTrue(await sortedTroves.contains(assetAddress1, alice))
    assert.isTrue(await sortedTroves.contains(assetAddress1, bob))
    assert.isTrue(await sortedTroves.contains(assetAddress1, carol))
    ...
    assert.equal(erin_Trove[TroveData.status], 3)
    assert.equal(freddy_Trove[TroveData.status], 3)
    assert.equal(greta_Trove[TroveData.status], 3)
    assert.equal(harry_Trove[TroveData.status], 3)
  ...
  it("liquidateTroves(): Liquidates Troves until 1) system has left recovery mode AND 2) it reaches a Trove with ICR >= 110%", async () => {
   ...
   (see calculations in Google Sheet)
    ...
    // check all other Troves are liquidated
    assert.equal(bob_Trove[TroveData.status], 3)
    assert.equal(carol_Trove[TroveData.status], 3)
    assert.equal(dennis_Trove[TroveData.status], 3)
    assert.equal(erin_Trove[TroveData.status], 3)
    assert.equal(freddy_Trove[TroveData.status], 3)
  ...
  it('liquidateTroves(): closes every Trove with ICR < MCR, when n > number of undercollateralized troves', async () => {
    ...
    await stabilityPool.provideToSP(dec(300, 18), erc20.address, { from: whale })
    ...
    assert.equal((await troveManager.Troves(freddy, assetAddress1))[TroveData.status].toString(), '3')
  })
  ...
  it('liquidateTroves():  emits liquidation event with correct values when all troves have ICR > 110% and Stability Pool covers a subset of troves, including a partial', async () => {
    ...
    const entireColl_A = (await troveManager.Troves(alice, assetAddress1))[TroveData.coll].add(await troveManager.getPendingReward(assetAddress1, alice))
    const entireDebt_A = (await troveManager.Troves(alice, assetAddress1))[TroveData.debt].add(await troveManager.getPendingKUSDDebtReward(assetAddress1, alice))
  ...
  it("liquidateTroves(): Liquidating troves at 100 < ICR < 110 with SP deposits correctly impacts their SP deposit and ETH gain", async () => {
    ...
    await stabilityPool.provideToSP(A_kusdAmount, erc20.address, { from: alice })
    await stabilityPool.provideToSP(B_kusdAmount, erc20.address, { from: bob })
  ...
  it("liquidateTroves(): Liquidating troves at ICR <=100% with SP deposits does not alter their deposit or ETH gain", async () => {
    ...
    await stabilityPool.provideToSP(dec(100, 18), erc20.address, { from: alice })
    await stabilityPool.provideToSP(dec(300, 18), erc20.address, { from: bob })
  ...
  it("liquidateTroves() with a non fullfilled liquidation: non liquidated trove remains active", async () => {
    ...
    assert.isTrue(await sortedTroves.contains(assetAddress1, carol))
    assert.equal((await troveManager.Troves(carol, assetAddress1))[TroveData.status].toString(), '1') // check Status is active
  })
  ...
  it("liquidateTroves() with a non fullfilled liquidation: non liquidated trove remains in TroveOwners Array", async () => {
    ...
    // Check TroveOwners idx on trove struct == idx of address found in TroveOwners array
    const idxOnStruct = (await troveManager.Troves(carol, assetAddress1))[TroveData.arrayIndex].toString()
    assert.equal(addressIdx.toString(), idxOnStruct)
  })
  ...
  it("batchLiquidateTroves(): Liquidates all troves with ICR < 110%, transitioning Normal -> Recovery Mode", async () => {
    ...
    */
    ...
    assert.equal(alice_Trove[TroveData.status], 2)
  ...
  it("batchLiquidateTroves(), with 110% < ICR < TCR, and StabilityPool empty: doesn't liquidate any troves", async () => {
    ...
    const bobColl_Before = (await troveManager.Troves(bob, assetAddress1))[TroveData.coll]
    const carolColl_Before = (await troveManager.Troves(carol, assetAddress1))[TroveData.coll]
    const dennisColl_Before = (await troveManager.Troves(dennis, assetAddress1))[TroveData.coll]
    ...
    assert.equal((await troveManager.Troves(dennis, assetAddress1))[TroveData.status], '1')
    ...
    const dennisDebt_After = (await troveManager.Troves(dennis, assetAddress1))[TroveData.debt].add(await troveManager.getPendingKUSDDebtReward(assetAddress1, dennis))
    const bobDebt_After = (await troveManager.Troves(bob, assetAddress1))[TroveData.debt].add(await troveManager.getPendingKUSDDebtReward(assetAddress1, bob))
    const carolDebt_After = (await troveManager.Troves(carol, assetAddress1))[TroveData.debt].add(await troveManager.getPendingKUSDDebtReward(assetAddress1, carol))

    const dennisColl_After = (await troveManager.Troves(dennis, assetAddress1))[TroveData.coll].add(await troveManager.getPendingReward(assetAddress1, dennis))
    const bobColl_After = (await troveManager.Troves(bob, assetAddress1))[TroveData.coll].add(await troveManager.getPendingReward(assetAddress1, bob))
    const carolColl_After = (await troveManager.Troves(carol, assetAddress1))[TroveData.coll].add(await troveManager.getPendingReward(assetAddress1, carol))
  ...
  it('batchLiquidateTroves(): skips liquidation of troves with ICR > TCR, regardless of Stability Pool size', async () => {
    ...
    const G_collBefore = (await troveManager.Troves(G, assetAddress1))[TroveData.coll]
    const G_debtBefore = (await troveManager.Troves(G, assetAddress1))[TroveData.debt]
    const H_collBefore = (await troveManager.Troves(H, assetAddress1))[TroveData.coll]
    const H_debtBefore = (await troveManager.Troves(H, assetAddress1))[TroveData.debt]
    const I_collBefore = (await troveManager.Troves(I, assetAddress1))[TroveData.coll]
    const I_debtBefore = (await troveManager.Troves(I, assetAddress1))[TroveData.debt]
