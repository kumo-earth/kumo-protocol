...
contract('ActivePool', async accounts => {
  ...
  it('increaseKUSD(): increases the recorded KUSD balance by the correct amount', async () => {
    ...
    const increaseKUSDDebtData = th.getTransactionData('increaseKUSDDebt(address,uint256)', [ZERO_ADDRESS, '0x64'])
  ...
  it('decreaseKUSD(): decreases the recorded KUSD balance by the correct amount', async () => {
    ...
    const decreaseKUSDDebtData = th.getTransactionData('decreaseKUSDDebt(address,uint256)', [ZERO_ADDRESS, '0x64'])
  ...
  it('sendAsset(): decreases the recorded Asset balance by the correct amount', async () => {
    ...
    const recieveERC20Asset = th.getTransactionData('receivedERC20(address,uint256)', [erc20Test.address, web3.utils.toHex(dec(2, 'ether'))])
    const tx1 = await mockBorrowerOperations.forward(activePool.address, recieveERC20Asset)

    assert.isTrue(tx1.receipt.status)

    const activePool_BalanceBeforeTx_ERC20 = web3.utils.toBN(await erc20Test.balanceOf(activePool.address))
    const activePool_BalanceBeforeTx_AssetBalance = await activePool.getAssetBalance(erc20Test.address)
    ...
    const sendAssetData = th.getTransactionData('sendAsset(address,address,uint256)', [erc20Test.address, alice, web3.utils.toHex(dec(1, 'ether'))])
    const tx2 = await mockBorrowerOperations.forward(activePool.address, sendAssetData, { from: owner })
    assert.isTrue(tx2.receipt.status)

    const activePool_BalanceAfterTx_ERC20 = web3.utils.toBN(await erc20Test.balanceOf(activePool.address))
    const activePool_BalanceAfterTx_AssetBalance = await activePool.getAssetBalance(erc20Test.address)
    const alice_Balance_AfterTx = web3.utils.toBN(await erc20Test.balanceOf(alice))

    const alice_BalanceChange = alice_Balance_AfterTx.sub(alice_Balance_BeforeTx)
    const pool_BalanceChange_ERC20 = activePool_BalanceAfterTx_ERC20.sub(activePool_BalanceBeforeTx_ERC20)
    const pool_BalanceChange_AssetBalance = activePool_BalanceAfterTx_AssetBalance.sub(activePool_BalanceBeforeTx_AssetBalance)
...
contract('DefaultPool', async accounts => {
  ...
  it('sendETHToActivePool(): decreases the recorded ETH balance by the correct amount', async () => {
    ...
    const recieveERC20Asset = th.getTransactionData('receivedERC20(address,uint256)', [erc20Test.address, web3.utils.toHex(dec(2, 'ether'))])
    const tx1 = await mockActivePool.forward(defaultPool.address, recieveERC20Asset, { from: owner })
    assert.isTrue(tx1.receipt.status)


    // const defaultPool_BalanceBeforeTx = web3.utils.toBN(await web3.eth.getBalance(defaultPool.address))
    const defaultPool_Balance_BeforeTx_ERC20 = web3.utils.toBN(await erc20Test.balanceOf(defaultPool.address))
    const defaultPool_Balance_BeforeTx_AssetBalance = await defaultPool.getAssetBalance(erc20Test.address)
    const activePool_Balance_BeforeTx = web3.utils.toBN(await erc20Test.balanceOf(mockActivePool.address))
    ...
    const sendAssetData = th.getTransactionData('sendAssetToActivePool(address,uint256)', [erc20Test.address, web3.utils.toHex(dec(1, 'ether'))])
    const tx2 = await mockTroveManager.forward(defaultPool.address, sendAssetData, { from: owner })
    assert.isTrue(tx2.receipt.status)


    const defaultPool_Balance_AfterTx_ERC20 = web3.utils.toBN(await erc20Test.balanceOf(defaultPool.address))
    const defaultPool_Balance_AfterTx_AssetBalance = await defaultPool.getAssetBalance(erc20Test.address)
    const activePool_Balance_AfterTx = web3.utils.toBN(await erc20Test.balanceOf(mockActivePool.address))

    const activePool_BalanceChange = activePool_Balance_AfterTx.sub(activePool_Balance_BeforeTx)
    const defaultPool_BalanceChange_ERC20 = defaultPool_Balance_AfterTx_ERC20.sub(defaultPool_Balance_BeforeTx_ERC20)
    const defaultPool_BalanceChange_AssetBalance = defaultPool_Balance_AfterTx_AssetBalance.sub(defaultPool_Balance_BeforeTx_AssetBalance)
