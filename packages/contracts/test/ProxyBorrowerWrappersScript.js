...
contract('BorrowerWrappers', async accounts => {
  ...
  it('claimCollateralAndOpenTrove(): reverts if nothing to claim', async () => {
    ...
    // alice opens Trove
    const { kusdAmount, collateral } = await openTrove({ ICR: toBN(dec(15, 17)), extraParams: { from: alice } })
    ...
    await assertRevert(
      borrowerWrappers.claimCollateralAndOpenTrove(th._100pct, kusdAmount, alice, alice, { from: alice }),
  ...
  it('claimCollateralAndOpenTrove(): without sending any value', async () => {
    // alice opens Trove
    const { kusdAmount, netDebt: redeemAmount, collateral } = await openTrove({extraKUSDAmount: 0, ICR: toBN(dec(3, 18)), extraParams: { from: alice } })
    // Whale opens Trove
    await openTrove({ extraKUSDAmount: redeemAmount, ICR: toBN(dec(5, 18)), extraParams: { from: whale } })
  ...
  it('claimCollateralAndOpenTrove(): sending value in the transaction', async () => {
    // alice opens Trove
    const { kusdAmount, netDebt: redeemAmount, collateral } = await openTrove({ extraParams: { from: alice } })
    // Whale opens Trove
    await openTrove({ extraKUSDAmount: redeemAmount, ICR: toBN(dec(2, 18)), extraParams: { from: whale } })
