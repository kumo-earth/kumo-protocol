...
contract('KUMOStaking revenue share tests', async accounts => {
  ...
  it("ETH fee per KUMO staked increases when a redemption fee is triggered and totalStakes > 0", async () => {
    ...
    // B redeems
    const redemptionTx = await th.redeemCollateralAndGetTxObject(assetAddress1, B, contracts, dec(100, 18))
  ...
  it("KUMO Staking: A single staker earns all ETH and KUMO fees that occur", async () => {
    ...
    const B_BalBeforeREdemption = await kusdToken.balanceOf(B)
    // B redeems
    const redemptionTx_1 = await th.redeemCollateralAndGetTxObject(assetAddress1, B, contracts, dec(100, 18))
    ...
    // C redeems
    const redemptionTx_2 = await th.redeemCollateralAndGetTxObject(assetAddress1, C, contracts, dec(100, 18))
  ...
  it("KUMO Staking: Multiple stakers earn the correct share of all Asset and KUMO fees, based on their stake size", async () => {
    ...
    // F redeems
    const redemptionTx_1 = await th.redeemCollateralAndGetTxObject(assetAddress1, F, contracts, dec(45, 18))
    const emittedETHFee_1 = toBN((await th.getEmittedRedemptionValues(redemptionTx_1))[3])
    assert.isTrue(emittedETHFee_1.gt(toBN('0')))

    // G redeems
    const redemptionTx_2 = await th.redeemCollateralAndGetTxObject(assetAddress1, G, contracts, dec(197, 18))
    const emittedETHFee_2 = toBN((await th.getEmittedRedemptionValues(redemptionTx_2))[3])
    assert.isTrue(emittedETHFee_2.gt(toBN('0')))
    ...
    // G redeems
    const redemptionTx_3 = await th.redeemCollateralAndGetTxObject(assetAddress1, C, contracts, dec(197, 18))
    ...
    */
    ...
    // Expected ETH gains
    const expectedAsset1Gain_A = toBN('100').mul(emittedETHFee_1).div(toBN('600'))
      .add(toBN('100').mul(emittedETHFee_2).div(toBN('600')))
      .add(toBN('100').mul(emittedETHFee_3).div(toBN('650')))

    const expectedAsset1Gain_B = toBN('200').mul(emittedETHFee_1).div(toBN('600'))
      .add(toBN('200').mul(emittedETHFee_2).div(toBN('600')))
      .add(toBN('200').mul(emittedETHFee_3).div(toBN('650')))

    const expectedAsset1Gain_C = toBN('300').mul(emittedETHFee_1).div(toBN('600'))
  ...
  it.skip("unstake(): reverts if caller has ETH gains and can't receive ETH", async () => {
    ...
    const redemptionTx_1 = await th.redeemCollateralAndGetTxObject(assetAddress1, B, contracts, dec(45, 18))

    const proxy_Asset1Gain = await kumoStaking.getPendingAssetGain(assetAddress1, nonPayable.address)
  ...
  it("receive(): reverts when it receives erc20 token from an address that is not the Active Pool", async () => {
    const asset1SendTxPromise1 = kumoStaking.receivedERC20(kumoStaking.address, dec(1, 'ether'), { from: A })
    const asset1SendTxPromise2 = kumoStaking.receivedERC20(kumoStaking.address, dec(1, 'ether'), { from: owner })
