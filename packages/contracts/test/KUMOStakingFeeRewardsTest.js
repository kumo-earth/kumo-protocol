...
contract('KUMOStaking revenue share tests', async accounts => {
  ...
  it("ETH fee per KUMO staked increases when a redemption fee is triggered and totalStakes > 0", async () => {
    ...
    // B redeems
    const redemptionTx = await th.redeemCollateralAndGetTxObject(B, contracts, dec(100, 18), assetAddress1)
  ...
  it("ETH fee per KUMO staked doesn't change when a redemption fee is triggered and totalStakes == 0", async () => {
    ...
    await openTrove({ asset: assetAddress1,  extraKUSDAmount: toBN(dec(30000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B} })
    await openTrove({ asset: assetAddress1,  extraKUSDAmount: toBN(dec(40000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C} })
    await openTrove({ asset: assetAddress1,  extraKUSDAmount: toBN(dec(50000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D} })
  ...
  it("KUSD fee per KUMO staked increases when a redemption fee is triggered and totalStakes > 0", async () => {
    ...
    // D draws debt
    const tx = await borrowerOperations.withdrawKUSD(assetAddress1, th._100pct, dec(27, 18), D, D, {from: D})
    ...
    assert.isTrue(expected_F_KUSD_After.add(expected_F_KUSD_After).eq(F_KUSD_After))
  })
  ...
  it("KUMO Staking: A single staker earns all ETH and KUMO fees that occur", async () => {
    ...
    const B_BalBeforeREdemption = await kusdToken.balanceOf(B)
    // B redeems
    const redemptionTx_1 = await th.redeemCollateralAndGetTxObject(B, contracts, dec(100, 18), assetAddress1)
    ...
    // C redeems
    const redemptionTx_2 = await th.redeemCollateralAndGetTxObject(C, contracts, dec(100, 18), assetAddress1)
    ...
    // D draws debt
    const borrowingTx_1 = await borrowerOperations.withdrawKUSD(assetAddress1, th._100pct, dec(104, 18), D, D, {from: D})
    ...
    // B draws debt
    const borrowingTx_2 = await borrowerOperations.withdrawKUSD(assetAddress1, th._100pct, dec(17, 18), B, B, {from: B})
  ...
  it("getPendingETHGain(): Returns the staker's correct pending ETH gain", async () => { 
    ...
    const A_ETHGain = await kumoStaking.getPendingAssetGain(assetAddress1, A)
  ...
  it("KUMO Staking: Multiple stakers earn the correct share of all ETH and KUMO fees, based on their stake size", async () => {
    ...
    await openTrove({ asset: assetAddress1,  extraKUSDAmount: toBN(dec(40000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
    await openTrove({ asset: assetAddress1,  extraKUSDAmount: toBN(dec(50000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
    await openTrove({ asset: assetAddress1,  extraKUSDAmount: toBN(dec(40000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })
    await openTrove({ asset: assetAddress1,  extraKUSDAmount: toBN(dec(50000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: F } })
    await openTrove({ asset: assetAddress1,  extraKUSDAmount: toBN(dec(50000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: G } })
    ...
    // F redeems
    const redemptionTx_1 = await th.redeemCollateralAndGetTxObject(F, contracts, dec(45, 18), assetAddress1)
    const emittedETHFee_1 = toBN((await th.getEmittedRedemptionValues(redemptionTx_1))[3])
    assert.isTrue(emittedETHFee_1.gt(toBN('0')))

     // G redeems
     const redemptionTx_2 = await th.redeemCollateralAndGetTxObject(G, contracts, dec(197, 18), assetAddress1)
     const emittedETHFee_2 = toBN((await th.getEmittedRedemptionValues(redemptionTx_2))[3])
     assert.isTrue(emittedETHFee_2.gt(toBN('0')))

    // F draws debt
    const borrowingTx_1 = await borrowerOperations.withdrawKUSD(assetAddress1, th._100pct, dec(104, 18), F, F, {from: F})
    const emittedKUSDFee_1 = toBN(th.getKUSDFeeFromKUSDBorrowingEvent(borrowingTx_1))
    assert.isTrue(emittedKUSDFee_1.gt(toBN('0')))

    // G draws debt
    const borrowingTx_2 = await borrowerOperations.withdrawKUSD(assetAddress1, th._100pct, dec(17, 18), G, G, {from: G})
    const emittedKUSDFee_2 = toBN(th.getKUSDFeeFromKUSDBorrowingEvent(borrowingTx_2))
    assert.isTrue(emittedKUSDFee_2.gt(toBN('0')))
    ...
    assert.equal(await kumoStaking.totalKUMOStaked(), dec(650, 18))
     ...
     // G redeems
     const redemptionTx_3 = await th.redeemCollateralAndGetTxObject(C, contracts, dec(197, 18), assetAddress1)
     const emittedETHFee_3 = toBN((await th.getEmittedRedemptionValues(redemptionTx_3))[3])
     assert.isTrue(emittedETHFee_3.gt(toBN('0')))

     // G draws debt
    const borrowingTx_3 = await borrowerOperations.withdrawKUSD(assetAddress1, th._100pct, dec(17, 18), G, G, {from: G})
    ...
    */
    ...
    const A_ETHBalance_Before = toBN(await erc20.balanceOf(A))
    const A_KUSDBalance_Before = toBN(await kusdToken.balanceOf(A))
    const B_ETHBalance_Before = toBN(await erc20.balanceOf(B))
    const B_KUSDBalance_Before = toBN(await kusdToken.balanceOf(B))
    const C_ETHBalance_Before = toBN(await erc20.balanceOf(C))
    const C_KUSDBalance_Before = toBN(await kusdToken.balanceOf(C))
    const D_ETHBalance_Before = toBN(await erc20.balanceOf(D))
    ...
    const A_ETHBalance_After = toBN(await erc20.balanceOf(A))
    const A_KUSDBalance_After = toBN(await kusdToken.balanceOf(A))
    const B_ETHBalance_After = toBN(await erc20.balanceOf(B))
    const B_KUSDBalance_After = toBN(await kusdToken.balanceOf(B))
    const C_ETHBalance_After = toBN(await erc20.balanceOf(C))
    const C_KUSDBalance_After = toBN(await kusdToken.balanceOf(C))
    const D_ETHBalance_After = toBN(await erc20.balanceOf(D))
    ...
    assert.isAtMost(th.getDifference(expectedKUSDGain_A.add(expectedKUSDGain_A), A_KUSDGain), 1000)
    assert.isAtMost(th.getDifference(expectedETHGain_B, B_ETHGain), 1000)
    assert.isAtMost(th.getDifference(expectedKUSDGain_B.add(expectedKUSDGain_B), B_KUSDGain), 1000)
    assert.isAtMost(th.getDifference(expectedETHGain_C, C_ETHGain), 1000)
    assert.isAtMost(th.getDifference(expectedKUSDGain_C.add(expectedKUSDGain_C), C_KUSDGain), 1000)
    assert.isAtMost(th.getDifference(expectedETHGain_D, D_ETHGain), 1000)
    assert.isAtMost(th.getDifference(expectedKUSDGain_D.add(expectedKUSDGain_D), D_KUSDGain), 1000)
  })
 
  it("unstake(): reverts if caller has ETH gains and can't receive ETH",  async () => {
    await openTrove({ asset: assetAddress1,  extraKUSDAmount: toBN(dec(20000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: whale } })  
    await openTrove({ asset: assetAddress1,  extraKUSDAmount: toBN(dec(20000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
    await openTrove({ asset: assetAddress1,  extraKUSDAmount: toBN(dec(30000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
    await openTrove({ asset: assetAddress1,  extraKUSDAmount: toBN(dec(40000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
    await openTrove({ asset: assetAddress1,  extraKUSDAmount: toBN(dec(50000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
    ...
    const proxyApproveTxData = await th.getTransactionData('approve(address,uint256)', [kumoStaking.address, '0x56bc75e2d63100000'])  // proxy stakes 100 VSTA
    ...
    const redemptionTx_1 = await th.redeemCollateralAndGetTxObject(B, contracts, dec(45, 18), assetAddress1)
    
    const proxy_ETHGain = await kumoStaking.getPendingAssetGain(assetAddress1, nonPayable.address)
