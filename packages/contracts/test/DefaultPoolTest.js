...
contract("DefaultPool", async accounts => {
  ...
  it("sendAssetToActivePool(): fails if receiver cannot receive ERC20 Asset", async () => {
    ...
    const recieveERC20Asset1 = th.getTransactionData("receivedERC20(address,uint256)", [
      erc20Test.address,
      web3.utils.toHex(amount)
    ]);
    const tx = await mockActivePool.forward(defaultPool.address, recieveERC20Asset1, {
      from: owner
    });
    assert.isTrue(tx.receipt.status);
