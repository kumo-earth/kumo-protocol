...
contract('Gas compensation tests', async accounts => {
  ...
  it('_getCollGasCompensation(): returns the 0.5% of collaterall if it is < $10 in value', async () => {
    ...
    const gasCompensation_2 = (await troveManager.getCollGasCompensation(erc20.address, dec(100, 'finney'))).toString()
    ...
    const gasCompensation_3 = (await troveManager.getCollGasCompensation(erc20.address, '5000000000')).toString()
  ...
  it('_getCollGasCompensation(): returns 0.5% of collaterall when 0.5% of collateral < $10 in value', async () => {
    ...
    -> Expect 0.5% of collaterall as gas compensation */
    const gasCompensation_1 = (await troveManager.getCollGasCompensation(erc20.address, '9999000000000000000')).toString()
    ...
    /* ETH:USD price = 200
     ...
     -> Expect 0.5% of collaterall as gas compensation */
    const gasCompensation_2 = (await troveManager.getCollGasCompensation(erc20.address, '55000000000000000')).toString()
    ...
    -> Expect 0.5% of collaterall as gas compensation */
    const gasCompensation_3 = (await troveManager.getCollGasCompensation(erc20.address, '6092324088087235800')).toString()
    assert.equal(gasCompensation_3, '30461620440436179')
  })

  it('getCollGasCompensation(erc20.address, ): returns 0.5% of collaterall when 0.5% of collateral = $10 in value', async () => {
    const price = await priceFeed.getPrice()
    assert.equal(price, dec(200, 18))
    ...
    -> Expect 0.5% of collaterall as gas compensation */
    const gasCompensation = (await troveManager.getCollGasCompensation(erc20.address, dec(10, 'ether'))).toString()
  ...
  it('getCollGasCompensation(): returns 0.5% of collaterall when 0.5% of collateral = $10 in value', async () => {
    ...
    const gasCompensation_1 = (await troveManager.getCollGasCompensation(erc20.address, dec(100, 'ether'))).toString()
    ...
    const gasCompensation_2 = (await troveManager.getCollGasCompensation(erc20.address, '10001000000000000000')).toString()
    ...
    const gasCompensation_3 = (await troveManager.getCollGasCompensation(erc20.address, '37500000000000000000')).toString()
    ...
    const gasCompensation_4 = await troveManager.getCollGasCompensation(erc20.address, '94758230582309850000000')
    ...
    const price_2 = await priceFeed.getPrice()
    const gasCompensation_5 = (await troveManager.getCollGasCompensation(erc20.address, '300000000000000000000000000')).toString()
  ...
  it('_getCompositeDebt(): returns (debt + 50) when collateral < $10 in value', async () => {
    ...
    const compositeDebt_1 = await troveManager.getCompositeDebt(erc20.address, dec(10, 18))
    ...
    const compositeDebt_2 = await troveManager.getCompositeDebt(erc20.address, 0)
    ...
    const compositeDebt_3 = await troveManager.getCompositeDebt(erc20.address, dec(200, 18))
  ...
  it('getCompositeDebt(): returns (debt + 50) collateral = $10 in value', async () => {
    ...
    const compositeDebt = await troveManager.getCompositeDebt(erc20.address, '123450000000000000000')
  ...
  it('getCompositeDebt(): returns (debt + 50) when 0.5% of collateral > $10 in value', async () => {
    ...
    const compositeDebt_1 = (await troveManager.getCompositeDebt(erc20.address, dec(2000, 18))).toString()
    ...
    debt = 200 KUSD
    -> Expect composite debt = (200 + 200) = 400 KUSD  */
    const compositeDebt_2 = (await troveManager.getCompositeDebt(erc20.address, dec(200, 18))).toString()
    ...
    const compositeDebt_3 = (await troveManager.getCompositeDebt(erc20.address, dec(500, 18))).toString()
    ...
    await priceFeed.setPrice('45323545420000000000000')
    const price_2 = await priceFeed.getPrice()
    const compositeDebt_4 = (await troveManager.getCompositeDebt(erc20.address, dec(1, 27))).toString()
   ...
   -> Expect composite debt = (54321.123456789 + 200) = 54521.123456789 KUSD */
    await priceFeed.setPrice(dec(1, 24))
    const price_3 = await priceFeed.getPrice()
    const compositeDebt_5 = (await troveManager.getCompositeDebt(erc20.address, '54321123456789000000000')).toString()
  ...
  it('getCurrentICR(): Incorporates virtual debt, and returns the correct ICR for new troves', async () => {
    ...
    const alice_ICR = (await troveManager.getCurrentICR(erc20.address, alice, price)).toString()
    ...
    const bob_ICR = (await troveManager.getCurrentICR(erc20.address, bob, price)).toString()
    // Expect Bob's ICR = (0.5 * 200) / 50 = 200%
    assert.isAtMost(th.getDifference(bob_ICR, dec(2, 18)), 1000)

    // F opens with 1 ETH, 100 KUSD
    await openTrove({ ICR: toBN(dec(2, 18)), extraKUSDAmount: dec(100, 18), extraParams: { from: flyn } })
    const flyn_ICR = (await troveManager.getCurrentICR(erc20.address, flyn, price)).toString()
    ...
    const carol_ICR = (await troveManager.getCurrentICR(erc20.address, carol, price)).toString()
    ...
    const dennis_ICR = (await troveManager.getCurrentICR(erc20.address, dennis, price)).toString()
    ...
    const erin_ICR = (await troveManager.getCurrentICR(erc20.address, erin, price)).toString()
    ...
    const harriet_ICR = (await troveManager.getCurrentICR(erc20.address, harriet, price)).toString()
  ...
  it('Gas compensation from pool-offset liquidations. All collateral paid as compensation', async () => {
    ...
    // A-E open troves
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(2, 18)), extraKUSDAmount: dec(100, 18), extraParams: { from: alice } })
    const { totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(2, 18)), extraKUSDAmount: dec(200, 18), extraParams: { from: bob } })
    const { totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(2, 18)), extraKUSDAmount: dec(300, 18), extraParams: { from: carol } })
    await openTrove({ ICR: toBN(dec(2, 18)), extraKUSDAmount: A_totalDebt, extraParams: { from: dennis } })
    await openTrove({ ICR: toBN(dec(2, 18)), extraKUSDAmount: B_totalDebt.add(C_totalDebt), extraParams: { from: erin} })

    // D, E each provide KUSD to SP
    await stabilityPool.provideToSP(A_totalDebt, erc20.address, { from: dennis, gasPrice: GAS_PRICE })
    await stabilityPool.provideToSP(B_totalDebt.add(C_totalDebt), erc20.address, { from: erin, gasPrice: GAS_PRICE })
  ...
  it('gas compensation from pool-offset liquidations: 0.5% collateral < $10 in value. Compensates $10 worth of collateral, liquidates the remainder', async () => {
    ...
    // A-E open troves
    await openTrove({ ICR: toBN(dec(2, 18)), extraKUSDAmount: dec(200, 18), extraParams: { from: alice} })
    await openTrove({ ICR: toBN(dec(120, 16)), extraKUSDAmount: dec(5000, 18), extraParams: { from: bob } })
    await openTrove({ ICR: toBN(dec(60, 18)), extraKUSDAmount: dec(600, 18), extraParams: { from: carol } })
    await openTrove({ ICR: toBN(dec(80, 18)), extraKUSDAmount: dec(1, 23), extraParams: { from: dennis } })
    await openTrove({ ICR: toBN(dec(80, 18)), extraKUSDAmount: dec(1, 23), extraParams: { from: erin } })

    // D, E each provide 10000 KUSD to SP
    await stabilityPool.provideToSP(dec(1, 23), erc20.address, { from: dennis , gasPrice: GAS_PRICE})
    await stabilityPool.provideToSP(dec(1, 23), erc20.address, { from: erin , gasPrice: GAS_PRICE})
    ...
    const aliceICR = await troveManager.getCurrentICR(erc20.address, alice, price_1)
    ...
    const bobICR = await troveManager.getCurrentICR(erc20.address, bob, price_2)
  ...
  it('gas compensation from pool-offset liquidations: 0.5% collateral > $10 in value. Compensates 0.5% of  collateral, liquidates the remainder', async () => {
    ...
    // A-E open troves
    await openTrove({ ICR: toBN(dec(2, 18)), extraKUSDAmount: dec(2000, 18), extraParams: { from: alice,} })
    await openTrove({ ICR: toBN(dec(1875, 15)), extraKUSDAmount: dec(8000, 18), extraParams: { from: bob,} })
    await openTrove({ ICR: toBN(dec(2, 18)), extraKUSDAmount: dec(600, 18), extraParams: { from: carol} })
    await openTrove({ ICR: toBN(dec(4, 18)), extraKUSDAmount: dec(1, 23), extraParams: { from: dennis} })
    await openTrove({ ICR: toBN(dec(4, 18)), extraKUSDAmount: dec(1, 23), extraParams: { from: erin} })
   ...
   and (37.5 - 0.1875 ETH) ETH remainder liquidated */
    ...
    const bobICR = await troveManager.getCurrentICR(erc20.address, bob, price_1)
  ...
  it('Gas compensation from pool-offset liquidations. Liquidation event emits the correct gas compensation and total liquidated coll and debt', async () => {
    ...
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(2, 18)), extraKUSDAmount: dec(100, 18), extraParams: { from: alice } })
    const { totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(2, 18)), extraKUSDAmount: dec(200, 18), extraParams: { from: bob } })
    await openTrove({ ICR: toBN(dec(2, 18)), extraKUSDAmount: dec(300, 18), extraParams: { from: carol } })
    await openTrove({ ICR: toBN(dec(2, 18)), extraKUSDAmount: A_totalDebt, extraParams: { from: dennis } })
    await openTrove({ ICR: toBN(dec(2, 18)), extraKUSDAmount: B_totalDebt, extraParams: { from: erin } })

    // D, E each provide KUSD to SP
    await stabilityPool.provideToSP(A_totalDebt, erc20.address, { from: dennis })
    await stabilityPool.provideToSP(B_totalDebt, erc20.address, { from: erin })
  ...
  it('gas compensation from pool-offset liquidations. Liquidation event emits the correct gas compensation and total liquidated coll and debt', async () => {
    ...
    // D, E each provide 10000 KUSD to SP
    await stabilityPool.provideToSP(dec(1, 23), erc20.address, { from: dennis })
    await stabilityPool.provideToSP(dec(1, 23), erc20.address, { from: erin })
  ...
  it('liquidateTroves(erc20.address, ): full offset.  Compensates the correct amount, and liquidates the remainder', async () => {
    await priceFeed.setPrice(dec(1000, 18))

    await openTrove({ ICR: toBN(dec(2000, 18)), extraParams: { from: whale } })

    // A-F open troves
    await openTrove({ ICR: toBN(dec(118, 16)), extraKUSDAmount: dec(2000, 18), extraParams: { from: alice } })
    await openTrove({ ICR: toBN(dec(526, 16)), extraKUSDAmount: dec(8000, 18), extraParams: { from: bob } })
    await openTrove({ ICR: toBN(dec(488, 16)), extraKUSDAmount: dec(600, 18), extraParams: { from: carol } })
    await openTrove({ ICR: toBN(dec(545, 16)), extraKUSDAmount: dec(1, 23), extraParams: { from: dennis } })
    await openTrove({ ICR: toBN(dec(10, 18)), extraKUSDAmount: dec(1, 23), extraParams: { from: erin } })
    await openTrove({ ICR: toBN(dec(10, 18)), extraKUSDAmount: dec(1, 23), extraParams: { from: flyn } })

    // D, E each provide 10000 KUSD to SP
    await stabilityPool.provideToSP(dec(1, 23), erc20.address, { from: erin })
    await stabilityPool.provideToSP(dec(1, 23), erc20.address, { from: flyn })
    ...
    const collGasCompensation = await troveManager.getCollGasCompensation(erc20.address, price)
    ...
    */
    ...
    const GAS_Used_Liquidator = th.gasUsed(await troveManager.liquidateTroves(erc20.address, 4, { from: liquidator, gasPrice: GAS_PRICE  }))
  ...
  it('liquidateTroves(erc20.address, ): full redistribution. Compensates the correct amount, and liquidates the remainder', async () => {
    await priceFeed.setPrice(dec(1000, 18))
    ...
    const KUSDinDefaultPool_0 = await defaultPool.getKUSDDebt(erc20.address) 
    ...
    // --- Check value of of A's collateral is < $10, and value of B,C,D collateral are > $10  ---
    const aliceColl = (await troveManager.Troves(alice, erc20.address))[1]
    const bobColl = (await troveManager.Troves(bob, erc20.address))[1]
    const carolColl = (await troveManager.Troves(carol, erc20.address))[1]
    const dennisColl = (await troveManager.Troves(dennis, erc20.address))[1]
    ...
    */
    ...
    const KUSDinDefaultPool_1 = await defaultPool.getKUSDDebt(erc20.address) 
  ...
  it('liquidateTroves(): full offset. Liquidation event emits the correct gas compensation and total liquidated coll and debt', async () => {
    ...
    // A-F open troves
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(118, 16)), extraKUSDAmount: dec(2000, 18), extraParams: { from: alice } })
    const { totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(526, 16)), extraKUSDAmount: dec(8000, 18), extraParams: { from: bob } })
    const { totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(488, 16)), extraKUSDAmount: dec(600, 18), extraParams: { from: carol } })
    const { totalDebt: D_totalDebt } = await openTrove({ ICR: toBN(dec(545, 16)), extraKUSDAmount: dec(1, 23), extraParams: { from: dennis } })
    await openTrove({ ICR: toBN(dec(10, 18)), extraKUSDAmount: dec(1, 23), extraParams: { from: erin } })
    await openTrove({ ICR: toBN(dec(10, 18)), extraKUSDAmount: dec(1, 23), extraParams: { from: flyn } })
    ...
    */
    ...
    // Liquidate troves A-D
    const liquidationTxData = await troveManager.liquidateTroves(erc20.address, 4, { from: liquidator, gasPrice: GAS_PRICE  })
  ...
  it('liquidateTroves(erc20.address, ): full redistribution. Liquidation event emits the correct gas compensation and total liquidated coll and debt', async () => {
    await priceFeed.setPrice(dec(1000, 18))
  ...
  it('Trove ordering: same collateral, decreasing debt. Price successively increases. Troves should maintain ordering by ICR', async () => {
    ...
    for (const account of _10_accounts) {
      ...
      await openTrove({ extraKUSDAmount: debtString, extraParams: { from: account, value: dec(30, 'ether') } })
    ...
    const firstColl = (await troveManager.Troves(_10_accounts[0], erc20.address))[1]
    ...
    while (price < 210) {
      ...
      const ICRList = []
      const coll_firstTrove = (await troveManager.Troves(_10_accounts[0], erc20.address))[0]
      const gasComp_firstTrove = (await troveManager.getCollGasCompensation(erc20.address, coll_firstTrove)).toString()

      for (account of _10_accounts) {
        // Check gas compensation is the same for all troves
        const coll = (await troveManager.Troves(account, erc20.address))[0]
        const gasCompensation = (await troveManager.getCollGasCompensation(erc20.address, coll)).toString()

        assert.equal(gasCompensation, gasComp_firstTrove)

        const ICR = await troveManager.getCurrentICR(erc20.address, account, price)
        ICRList.push(ICR)
  ...
  it('Trove ordering: increasing collateral, constant debt. Price successively increases. Troves should maintain ordering by ICR', async () => {
    ...
    for (const account of _20_accounts) {
      ...
      await openTrove({ extraKUSDAmount: dec(100, 18), extraParams: { from: account, value: collString } })
