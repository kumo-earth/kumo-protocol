...
contract("PoolManager - random liquidations/deposits, then check all depositors can withdraw", async accounts => {
  ...
  const performLiquidation = async (remainingDefaulters, liquidatedAccountsDict) => {
    ...
    const liquidatedKUSD = (await troveManager.Troves(randomDefaulter))[0]
    ...
    const KUSDinPoolBefore = await stabilityPool.getTotalKUSDDeposits()
    const liquidatedTx = await troveManager.liquidate(randomDefaulter, { from: accounts[0] })
    const KUSDinPoolAfter = await stabilityPool.getTotalKUSDDeposits()
    ...
    console.log(`Liquidation. addr: ${th.squeezeAddr(randomDefaulter)} ICR: ${ICRPercent}% coll: ${liquidatedETH} debt: ${liquidatedKUSD} SP KUSD before: ${KUSDinPoolBefore} SP KUSD after: ${KUSDinPoolAfter} tx success: ${liquidatedTx.receipt.status}`)
  }
  ...
  const performSPDeposit = async (depositorAccounts, currentDepositors, currentDepositorsDict) => {
    ...
    const userBalance = (await kusdToken.balanceOf(randomDepositor))
    const maxKUSDDeposit = userBalance.div(toBN(dec(1, 18)))

    const randomKUSDAmount = th.randAmountInWei(1, maxKUSDDeposit)

    const depositTx = await stabilityPool.provideToSP(randomKUSDAmount, ZERO_ADDRESS, { from: randomDepositor })
    ...
    console.log(`SP deposit. addr: ${th.squeezeAddr(randomDepositor)} amount: ${randomKUSDAmount} tx success: ${depositTx.receipt.status} `)
  }
  ...
  const clearAllUndercollateralizedTroves = async (price) => {
    ...
    */
    while(await systemContainsTroveUnder100(price) && await troveManager.checkRecoveryMode()) {
      ...
      await kusdToken.transfer(lowestTrove, lowestTroveDebt, {from: whale})
  ...
  const attemptWithdrawAllDeposits = async (currentDepositors) => {
    ...
    for (depositor of currentDepositors) {
      ...
      const finalDeposit = await stabilityPool.getCompoundedKUSDDeposit(depositor)
      ...
      const KUSDinSPAfter = (await stabilityPool.getTotalKUSDDeposits()).toString()
      const KUSDBalanceSPAfter = (await kusdToken.balanceOf(stabilityPool.address))
      const depositAfter = await stabilityPool.getCompoundedKUSDDeposit(depositor)
      ...
      console.log(`--Before withdrawal--
                    ...
                    --After withdrawal--
                     ...
                     SP KUSD deposits tracker after: ${KUSDinSPAfter}
                     SP KUSD balance after: ${KUSDBalanceSPAfter}
                     `)
  ...
  describe("Stability Pool Withdrawals", async () => {
    ...
    it("Defaulters' Collateral in range [1, 1e8]. SP Deposits in range [100, 1e10]. ETH:USD = 100", async () => {
      ...
      const defaulterKUSDProportionMin = 91
      const defaulterKUSDProportionMax = 180

      const depositorCollMin = 1
      const depositorCollMax = 100000000
      const depositorKUSDProportionMin = 100
      const depositorKUSDProportionMax = 100
      ...
      // setup:
      // account set L all add coll and withdraw KUSD
      await th.openTrove_allAccounts_randomETH_randomKUSD(defaulterCollMin,
        defaulterCollMax,
        defaulterAccounts,
        contracts,
        defaulterKUSDProportionMin,
        defaulterKUSDProportionMax,
        true)

      // account set S all add coll and withdraw KUSD
      await th.openTrove_allAccounts_randomETH_randomKUSD(depositorCollMin,
        depositorCollMax,
        depositorAccounts,
        contracts,
        depositorKUSDProportionMin,
        depositorKUSDProportionMax,
        true)
      ...
      const totalKUSDDepositsBeforeWithdrawals = await stabilityPool.getTotalKUSDDeposits()
      const totalETHRewardsBeforeWithdrawals = await stabilityPool.getETH()

      await attemptWithdrawAllDeposits(currentDepositors)

      const totalKUSDDepositsAfterWithdrawals = await stabilityPool.getTotalKUSDDeposits()
      const totalETHRewardsAfterWithdrawals = await stabilityPool.getETH()

      console.log(`Total KUSD deposits before any withdrawals: ${totalKUSDDepositsBeforeWithdrawals}`)
      console.log(`Total ETH rewards before any withdrawals: ${totalETHRewardsBeforeWithdrawals}`)

      console.log(`Remaining KUSD deposits after withdrawals: ${totalKUSDDepositsAfterWithdrawals}`)
