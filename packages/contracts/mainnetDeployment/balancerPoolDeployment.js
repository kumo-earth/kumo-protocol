...
const KUSD = '0x5f98805A4E8be255a32880FDeC7F6728C6568bA0';
const CHAINLINK_ETHUSD_PROXY = '0x5f4eC3Df9cbd43714FE2740f5E3616155c5b8419';

const tokens = [KUSD, WETH];
const weights = [toBigNum(dec(4, 17)), toBigNum(dec(6, 17))];

const NAME = 'WETH/KUSD Pool';
const SYMBOL = '60WETH-40KUSD';
...
async function main() {
  ...
  */
  ...
  const kusd_balance = INITIAL_FUNDING.mul(weights[0]).div(toBigNum(dec(1, 18)));
  const initialBalances = [kusd_balance, weth_balance];
  th.logBN('Initial KUSD', kusd_balance);
  ...
  // Approve KUSD
  const kusd = new ethers.Contract(
    KUSD,
    ERC20.abi,
    deployerWallet
  );
  const txApproveKusd = await kusd.approve(VAULT, kusd_balance);
  const approveKusdReceipt = await txApproveKusd.wait();
  console.log('Approve KUSD gas: ', approveKusdReceipt.gasUsed.toString());
  ...
  console.log('Total gas: ', receipt1.gasUsed.add(depositWethReceipt.gasUsed).add(approveWethReceipt.gasUsed).add(approveKusdReceipt.gasUsed).add(receipt2.gasUsed).toString());
}
