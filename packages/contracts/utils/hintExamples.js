...
async function main() {
  ...
  const coreContracts = await dh.deployKumoCoreHardhat()
 ...
 const { troveManager, borrowerOperations, hintHelpers, sortedTroves, priceFeedTestnet } = coreContracts
  ...
  const KUSDAmount = toBN(toWei('2500')) // borrower wants to withdraw 2500 KUSD
  ...
  const liquidationReserve = await troveManager.KUSD_GAS_COMPENSATION()
  const expectedFee = await troveManager.getBorrowingFeeWithDecay(KUSDAmount)
  
  // Total debt of the new trove = KUSD amount drawn, plus fee, plus the liquidation reserve
  const expectedDebt = KUSDAmount.add(expectedFee).add(liquidationReserve)
  ...
  await borrowerOperations.openTrove(maxFee, KUSDAmount, upperHint, lowerHint, { value: ETHColl })

  // --- adjust trove --- 

  const collIncrease = toBN(toWei('1'))  // borrower wants to add 1 ETH
  const KUSDRepayment = toBN(toWei('230')) // borrower wants to repay 230 KUSD

  // Get trove's current debt and coll
  const {0: debt, 1: coll} = await troveManager.getEntireDebtAndColl(borrower)
  
  const newDebt = debt.sub(KUSDRepayment)
  ...
  await borrowerOperations.adjustTrove(maxFee, 0, KUSDRepayment, false, upperHint, lowerHint, {value: collIncrease})


  // --- RedeemCollateral ---

  // Get the redemptions hints from the deployed HintHelpers contract
  const redemptionhint = await hintHelpers.getRedemptionHints(KUSDAmount, price, 50)

  const {0: firstRedemptionHint, 1: partialRedemptionNewICR, 2: truncatedKUSDAmount} = redemptionhint
  ...
  */
  ...
  /* Finally, perform the on-chain redemption, passing the truncated KUSD amount, the correct hints, and the expected
  * ICR of the final partially redeemed trove in the sequence. 
  */
  await troveManager.redeemCollateral(truncatedKUSDAmount,
    firstRedemptionHint,
