...
$ hardhat test
  ...
  Contract: Access Control: Kumo functions with the caller restricted to Kumo contract(s)
    BorrowerOperations
      ✓ moveAssetGainToTrove(): reverts when called by an account that is not StabilityPool
    TroveManager
    ...
    ActivePool
      ...
      ✓ increaseKUSDDebt(): reverts when called by an account that is not BO nor TroveM
      ✓ decreaseKUSDDebt(): reverts when called by an account that is not BO nor TroveM nor SP
    ...
    DefaultPool
      ...
      ✓ increaseKUSDDebt(): reverts when called by an account that is not TroveManager
      ✓ decreaseKUSD(): reverts when called by an account that is not TroveManager
    ...
    LockupContract
      ✓ withdrawKUMO(): reverts when caller is not beneficiary (68ms)
    KUMOStaking
      ✓ increaseF_KUSD(): reverts when caller is not TroveManager
    KUMOToken
      ✓ sendToKUMOStaking(): reverts when caller is not the KUMOSstaking (49ms)
    CommunityIssuance
      ✓ sendKUMO(): reverts when caller is not the StabilityPool
      ✓ issueKUMO(): reverts when caller is not the StabilityPool
  ...
  Contract: BorrowerOperations
    Without proxy
      ...
      ✓ addColl(), active Trove: applies pending rewards and updates user's L_ETH, L_KUSDDebt snapshots (736ms)
      ...
      ✓ withdrawColl(): applies pending rewards and updates user's L_ETH, L_KUSDDebt snapshots (1205ms)
      ✓ withdrawKUSD(): reverts when withdrawal would leave trove with ICR < MCR (516ms)
      ✓ withdrawKUSD(): decays a non-zero base rate (1192ms)
      ✓ withdrawKUSD(): reverts if max fee > 100% (668ms)
      ✓ withdrawKUSD(): reverts if max fee < 0.5% in Normal mode (639ms)
      ✓ withdrawKUSD(): reverts if fee exceeds max fee percentage (938ms)
      ✓ withdrawKUSD(): succeeds when fee is less than max fee percentage (1272ms)
      ✓ withdrawKUSD(): doesn't change base rate if it is already zero (1044ms)
      ✓ withdrawKUSD(): lastFeeOpTime doesn't update if less time than decay interval has passed since the last fee operation (899ms)
      ✓ withdrawKUSD(): borrower can't grief the baseRate and stop it decaying by issuing debt at higher frequency than the decay granularity (852ms)
      ✓ withdrawKUSD(): borrowing at non-zero base rate sends KUSD fee to KUMO staking contract (1004ms)
      ✓ withdrawKUSD(): borrowing at non-zero base records the (drawn debt + fee) on the Trove struct (923ms)
      ✓ withdrawKUSD(): Borrowing at non-zero base rate increases the KUMO staking contract KUSD fees-per-unit-staked (986ms)
      ✓ withdrawKUSD(): Borrowing at non-zero base rate sends requested amount to the user (1562ms)
      ✓ withdrawKUSD(): Borrowing at zero base rate changes KUSD fees-per-unit-staked (839ms)
      ✓ withdrawKUSD(): Borrowing at zero base rate sends debt request to user (790ms)
      ✓ withdrawKUSD(): reverts when calling address does not have active trove (370ms)
      ✓ withdrawKUSD(): reverts when requested withdrawal amount is zero KUSD (459ms)
      ✓ withdrawKUSD(): reverts when system is in Recovery Mode (649ms)
      ✓ withdrawKUSD(): reverts when withdrawal would bring the trove's ICR < MCR (347ms)
      ✓ withdrawKUSD(): reverts when a withdrawal would cause the TCR of the system to fall below the CCR (439ms)
      ✓ withdrawKUSD(): reverts if system is in Recovery Mode (312ms)
      ✓ withdrawKUSD(): increases the Trove's KUSD debt by the correct amount (208ms)
      ✓ withdrawKUSD(): increases KUSD debt in ActivePool by correct amount (230ms)
      ✓ withdrawKUSD(): increases user KUSDToken balance by correct amount (243ms)
      ✓ repayKUSD(): reverts when repayment would leave trove with ICR < MCR (392ms)
      ✓ repayKUSD(): Succeeds when it would leave trove with net debt >= minimum net debt (435ms)
      ✓ repayKUSD(): reverts when it would leave trove with net debt < minimum net debt (256ms)
      ✓ repayKUSD(): reverts when calling address does not have active trove (431ms)
      ✓ repayKUSD(): reverts when attempted repayment is > the debt of the trove (418ms)
      ✓ repayKUSD(): reduces the Trove's KUSD debt by the correct amount (403ms)
      ✓ repayKUSD(): decreases KUSD debt in ActivePool by correct amount (406ms)
      ✓ repayKUSD(): decreases user KUSDToken balance by correct amount (404ms)
      ✓ repayKUSD(): can repay debt in Recovery Mode (557ms)
      ✓ repayKUSD(): Reverts if borrower has insufficient KUSD balance to cover his debt repayment (1122ms)
      ...
      ✓ adjustTrove(): borrowing at non-zero base rate sends KUSD fee to KUMO staking contract (889ms)
      ✓ adjustTrove(): borrowing at non-zero base records the (drawn debt + fee) on the Trove struct (1013ms)
      ✓ adjustTrove(): Borrowing at non-zero base rate increases the KUMO staking contract KUSD fees-per-unit-staked (837ms)
      ✓ adjustTrove(): Borrowing at non-zero base rate sends requested amount to the user (895ms)
      ✓ adjustTrove(): Borrowing at zero base rate changes KUSD balance of KUMO staking contract (783ms)
      ✓ adjustTrove(): Borrowing at zero base rate changes KUMO staking contract KUSD fees-per-unit-staked (881ms)
      ✓ adjustTrove(): Borrowing at zero base rate sends total requested KUSD to the user (783ms)
      ...
      ✓ adjustTrove(): reverts when KUSD repaid is > debt of the trove (451ms)
      ...
      ✓ adjustTrove(): changes KUSDToken balance by the requested decrease (529ms)
      ✓ adjustTrove(): changes KUSDToken balance by the requested increase (577ms)
      ...
      ✓ adjustTrove(): Changes the KUSD debt in ActivePool by requested decrease (541ms)
      ✓ adjustTrove(): Changes the KUSD debt in ActivePool by requested increase (397ms)
      ...
      ✓ adjustTrove(): Reverts if borrower has insufficient KUSD balance to cover his debt repayment (496ms)
      ...
      ✓ closeTrove(): subtracts the debt of the closed Trove from the Borrower's KUSDToken balance (405ms)
      ✓ closeTrove(): applies pending rewards (1267ms)
      ✓ closeTrove(): reverts if borrower has insufficient KUSD balance to repay his entire debt (347ms)
      ...
      ✓ openTrove(): borrowing at non-zero base rate sends KUSD fee to KUMO staking contract (923ms)
      ✓ openTrove(): borrowing at non-zero base records the (drawn debt + fee  + liq. reserve) on the Trove struct (865ms)
      ✓ openTrove(): Borrowing at non-zero base rate increases the KUMO staking contract KUSD fees-per-unit-staked (794ms)
      ✓ openTrove(): Borrowing at non-zero base rate sends requested amount to the user (832ms)
      ✓ openTrove(): Borrowing at zero base rate changes the KUMO staking contract KUSD fees-per-unit-staked (602ms)
      ...
      ✓ openTrove(): records up-to-date initial snapshots of L_ETH and L_KUSDDebt (639ms)
      ✓ openTrove(): allows a user to open a Trove, then close it, then re-open it (687ms)
      ✓ openTrove(): increases the Trove's KUSD debt by the correct amount (129ms)
      ✓ openTrove(): increases KUSD debt in ActivePool by the debt of the trove (162ms)
      ✓ openTrove(): increases user KUSDToken balance by correct amount (85ms)
  ...
  Contract: Deployment script - Sets correct contract addresses dependencies after deployment
    ...
    ✓ Sets the correct KUSDToken address in TroveManager
    ...
    ✓ Sets the correct KUMOStaking address in TroveManager
    ...
    ✓ Sets the correct KUSDToken address in StabilityPool
    ...
    ✓ Sets the correct KUMOStaking address in BorrowerOperations
    ✓ Sets the correct KUMOToken address in KUMOStaking
    ✓ Sets the correct ActivePool address in KUMOStaking
    ✓ Sets the correct ActivePool address in KUMOStaking
    ✓ Sets the correct ActivePool address in KUMOStaking
    ✓ Sets the correct BorrowerOperations address in KUMOStaking
    ✓ Sets the correct CommunityIssuance address in KUMOToken
    ✓ Sets the correct KUMOStaking address in KUMOToken
    ✓ Sets the correct LockupContractFactory address in KUMOToken
    ✓ Sets the correct KUMOToken address in LockupContractFactory
    ✓ Sets the correct KUMOToken address in CommunityIssuance
...
TCR: 11.892415157517211309
  ...
  Contract: KUMO Token
    ...
    ✓ sendToKUMOStaking(): changes balances of KUMOStaking and calling account by the correct amounts (48ms)
  ...
  Contract: Deploying and funding One Year Lockup Contracts
    Deploying LCs
      ✓ KUMO Deployer can deploy LCs through the Factory (121ms)
      ✓ Anyone can deploy LCs through the Factory (71ms)
      ✓ KUMO Deployer can deploy LCs directly (43ms)
    ...
    Funding LCs
      ✓ KUMO transfer from KUMO deployer to their deployed LC increases the KUMO balance of the LC (397ms)
      ✓ KUMO Multisig can transfer KUMO to LCs deployed through the factory by anyone (168ms)
    Withdrawal attempts on funded, inactive LCs immediately after funding
      ...
      ✓ KUMO multisig can't withraw from a LC which it funded (271ms)
      ✓ No one can withraw from a LC (78ms)

  Contract: Deploying the KUMO contracts: LCF, CI, KUMOStaking, and KUMOToken 
    CommunityIssuance deployment
      ✓ Stores the deployer's address
    KUMOStaking deployment
      ✓ Stores the deployer's address
    KUMOToken deployment
      ✓ Stores the multisig's address
      ✓ Stores the CommunityIssuance address
      ✓ Stores the LockupContractFactory address
      ✓ Mints the correct KUMO amount to the multisig's address: (64.66 million)
      ✓ Mints the correct KUMO amount to the CommunityIssuance contract address: 32 million
      ✓ Mints the correct KUMO amount to the bountyAddress EOA: 2 million
      ✓ Mints the correct KUMO amount to the lpRewardsAddress EOA: 1.33 million
    ...
    Connecting KUMOToken to LCF, CI and KUMOStaking
      ...
      ✓ sets the correct KUMOToken address in LockupContractFactory
      ✓ sets the correct KUMOToken address in CommunityIssuance (203ms)
  ...
  Contract: After the initial lockup period has passed
    Deploying new LCs
      ✓ KUMO Deployer can deploy new LCs
    ...
    Beneficiary withdrawal from initial LC
      ...
      ✓ Beneficiaries can withraw full KUMO balance of LC if it has increased since lockup period ended (234ms)
    Withdrawal attempts from LCs by non-beneficiaries
      ✓ KUMO Multisig can't withdraw from a LC they deployed through the Factory
      ✓ KUMO Multisig can't withdraw from a LC that someone else deployed
      ✓ Non-beneficiaries cannot withdraw from a LC (189ms)
    Transferring KUMO
      ✓ KUMO multisig can transfer KUMO to LCs they deployed (176ms)
      ✓ KUMO multisig can transfer tokens to LCs deployed by anyone (104ms)
      ✓ KUMO multisig can transfer KUMO directly to any externally owned account (141ms)
    ...
    Withdrawal Attempts on new LCs before unlockTime has passed
      ✓ KUMO Deployer can't withdraw from a funded LC they deployed for another beneficiary through the Factory, before the unlockTime (44ms)
      ✓ KUMO Deployer can't withdraw from a funded LC that someone else deployed, before the unlockTime (448ms)
    ...
    Withdrawals from new LCs after unlockTime has passed
      ✓ KUMO Deployer can't withdraw from a funded LC they deployed for another beneficiary through the Factory, after the unlockTime (100ms)
      ✓ KUMO multisig can't withdraw from a funded LC when they are not the beneficiary, after the unlockTime (529ms)
  ...
  Contract: KUMO community issuance arithmetic tests
...
issuance fraction after: 949066037374286
    ...
    ✓ Total KUMO tokens issued is 42.20 after a minute (38ms)
    ✓ Total KUMO tokens issued is 2,531.94 after an hour (41ms)
    ✓ Total KUMO tokens issued is 60,711.40 after a day (136ms)
    ✓ Total KUMO tokens issued is 422,568.60 after a week (331ms)
    ✓ Total KUMO tokens issued is 1,772,113.21 after a month
    ✓ Total KUMO tokens issued is 5,027,363.22 after 3 months (40ms)
    ✓ Total KUMO tokens issued is 9,264,902.04 after 6 months (47ms)
    ✓ Total KUMO tokens issued is 16,000,000 after a year (47ms)
    ✓ Total KUMO tokens issued is 24,000,000 after 2 years (47ms)
    ✓ Total KUMO tokens issued is 28,000,000 after 3 years (75ms)
    ✓ Total KUMO tokens issued is 30,000,000 after 4 years (40ms)
    ✓ Total KUMO tokens issued is 31,968,750 after 10 years (69ms)
    ✓ Total KUMO tokens issued is 31,999,969.48 after 20 years (68ms)
    ✓ Total KUMO tokens issued is 31,999,999.97 after 30 years (87ms)
  ...
  Contract: KUMOStaking revenue share tests
    ✓ stake(): reverts if amount is zero (74ms)
    ✓ ETH fee per KUMO staked increases when a redemption fee is triggered and totalStakes > 0 (1562ms)
    ✓ ETH fee per KUMO staked doesn't change when a redemption fee is triggered and totalStakes == 0 (3540ms)
    ✓ KUSD fee per KUMO staked increases when a redemption fee is triggered and totalStakes > 0 (1757ms)
    ✓ KUSD fee per KUMO staked doesn't change when a redemption fee is triggered and totalStakes == 0 (1705ms)
    ✓ KUMO Staking: A single staker earns all ETH and KUMO fees that occur (2143ms)
    ✓ stake(): Top-up sends out all accumulated ETH and KUSD gains to the staker (2138ms)
    ✓ getPendingETHGain(): Returns the staker's correct pending ETH gain (1856ms)
    ✓ getPendingKUSDGain(): Returns the staker's correct pending KUSD gain (1965ms)
    ✓ KUMO Staking: Multiple stakers earn the correct share of all ETH and KUMO fees, based on their stake size (3133ms)
  ...
  Contract: All Kumo functions with onlyOwner modifier
    ...
    LockupContractFactory
      ✓ setKUMOAddress(): reverts when called by non-owner, with wrong address, or twice (216ms)

  Contract: StabilityPool
    ✓ getETH(): gets the recorded ETH balance
    ✓ getTotalKUSDDeposits(): gets the recorded KUSD balance

  Contract: ActivePool
    ✓ getETH(): gets the recorded ETH balance
    ✓ getKUSDDebt(): gets the recorded KUSD balance
    ✓ increaseKUSD(): increases the recorded KUSD balance by the correct amount
    ✓ decreaseKUSD(): decreases the recorded KUSD balance by the correct amount
    ✓ sendETH(): decreases the recorded ETH balance by the correct amount

  Contract: DefaultPool
    ✓ getETH(): gets the recorded KUSD balance
    ✓ getKUSDDebt(): gets the recorded KUSD balance
    ✓ increaseKUSD(): increases the recorded KUSD balance by the correct amount
    ✓ decreaseKUSD(): decreases the recorded KUSD balance by the correct amount (57ms)
  ...
  Contract: BorrowerWrappers
    ...
    ✓ claimStakingGainsAndRecycle(): with only KUSD gain (821ms)
    ✓ claimStakingGainsAndRecycle(): with both ETH and KUSD gains (1593ms)
  ...
  Contract: StabilityPool - KUMO Rewards
    KUMO Rewards
totalKUMOIssued_1: 30370113195977152000000
totalKUMOIssued_2: 30370113195977152000000
      ✓ liquidation < 1 minute after a deposit does not change totalKUMOIssued (743ms)
      ✓ withdrawFromSP(): reward term G does not update when no KUMO is issued (634ms)
      ✓ withdrawFromSP(): Depositors with equal initial deposit withdraw correct KUMO gain. No liquidations. No front end. (1479ms)
      ✓ withdrawFromSP(): Depositors with varying initial deposit withdraw correct KUMO gain. No liquidations. No front end. (1345ms)
      ✓ withdrawFromSP(): Depositors with varying initial deposit withdraw correct KUMO gain. No liquidations. No front end. (1925ms)
      ✓ withdrawFromSP(): Depositor withdraws correct KUMO gain after serial pool-emptying liquidations. No front-ends. (5752ms)
      ✓ KUMO issuance for a given period is not obtainable if the SP was empty during the period (551ms)
      ✓ withdrawFromSP(): Several deposits of 100 KUSD span one scale factor change. Depositors withdraw correct KUMO gains (6075ms)
      ✓ withdrawFromSP(): Depositors with equal initial deposit withdraw correct KUMO gain. No liquidations. Front ends and kickback rates. (1660ms)
      ✓ withdrawFromSP(): Depositors with varying initial deposit withdraw correct KUMO gain. Front ends and kickback rates (2813ms)
      ✓ withdrawFromSP(): Several deposits of 10k KUSD span one scale factor change. Depositors withdraw correct KUMO gains (4788ms)

  Contract: Pool Manager: Sum-Product rounding errors
    - Rounding errors: 100 deposits of 100KUSD into SP, then 200 liquidations of 49KUSD
  ...
  Contract: StabilityPool - Withdrawal of stability deposit - Reward calculations
    Stability Pool Withdrawal
      ...
      ✓ withdrawFromSP(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after two liquidations of increasing KUSD (965ms)
      ✓ withdrawFromSP(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after three liquidations of increasing KUSD (1152ms)
      ...
      ✓ withdrawFromSP(): A, B, C Deposit -> 2 liquidations -> D deposits -> 1 liquidation. All deposits and liquidations = 100 KUSD.  A, B, C, D withdraw correct KUSD deposit and ETH Gain (1512ms)
      ✓ withdrawFromSP(): A, B, C Deposit -> 2 liquidations -> D deposits -> 2 liquidations. All deposits and liquidations = 100 KUSD.  A, B, C, D withdraw correct KUSD deposit and ETH Gain (1637ms)
      ✓ withdrawFromSP(): A, B, C Deposit -> 2 liquidations -> D deposits -> 2 liquidations. Various deposit and liquidation vals.  A, B, C, D withdraw correct KUSD deposit and ETH Gain (1645ms)
      ✓ withdrawFromSP(): A, B, C, D deposit -> 2 liquidations -> D withdraws -> 2 liquidations. All deposits and liquidations = 100 KUSD.  A, B, C, D withdraw correct KUSD deposit and ETH Gain (1704ms)
      ✓ withdrawFromSP(): A, B, C, D deposit -> 2 liquidations -> D withdraws -> 2 liquidations. Various deposit and liquidation vals. A, B, C, D withdraw correct KUSD deposit and ETH Gain (1753ms)
      ✓ withdrawFromSP(): A, B, D deposit -> 2 liquidations -> C makes deposit -> 1 liquidation -> D withdraws -> 1 liquidation. All deposits: 100 KUSD. Liquidations: 100,100,100,50.  A, B, C, D withdraw correct KUSD deposit and ETH Gain (1537ms)
...
alice deposit: 0
      ...
      ✓ withdrawFromSP(): Several deposits of 10000 KUSD span one scale factor change. Depositors withdraws correct compounded deposit and ETH Gain after one liquidation (1676ms)
  ...
  Contract: StabilityPool - Withdrawal of stability deposit - Reward calculations
    Stability Pool Withdrawal
      ✓ withdrawAssetGainToTrove(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after one liquidation (1227ms)
      ✓ withdrawAssetGainToTrove(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after two identical liquidations (1400ms)
      ✓ withdrawAssetGainToTrove():  Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after three identical liquidations (1827ms)
      ✓ withdrawAssetGainToTrove(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after two liquidations of increasing KUSD (1597ms)
      ✓ withdrawAssetGainToTrove(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after three liquidations of increasing KUSD (1711ms)
      ✓ withdrawAssetGainToTrove(): Depositors with varying deposits withdraw correct compounded deposit and ETH Gain after two identical liquidations (1782ms)
      ✓ withdrawAssetGainToTrove(): Depositors with varying deposits withdraw correct compounded deposit and ETH Gain after three identical liquidations (1798ms)
      ✓ withdrawAssetGainToTrove(): Depositors with varying deposits withdraw correct compounded deposit and ETH Gain after three varying liquidations (4488ms)

      ✓ withdrawAssetGainToTrove(): A, B, C Deposit -> 2 liquidations -> D deposits -> 1 liquidation. All deposits and liquidations = 100 KUSD.  A, B, C, D withdraw correct KUSD deposit and ETH Gain (2168ms)
      ✓ withdrawAssetGainToTrove(): A, B, C Deposit -> 2 liquidations -> D deposits -> 2 liquidations. All deposits and liquidations = 100 KUSD.  A, B, C, D withdraw correct KUSD deposit and ETH Gain (5085ms)
      ✓ withdrawAssetGainToTrove(): A, B, C Deposit -> 2 liquidations -> D deposits -> 2 liquidations. Various deposit and liquidation vals.  A, B, C, D withdraw correct KUSD deposit and ETH Gain (2304ms)
      ✓ withdrawAssetGainToTrove(): A, B, C, D deposit -> 2 liquidations -> D withdraws -> 2 liquidations. All deposits and liquidations = 100 KUSD.  A, B, C, D withdraw correct KUSD deposit and ETH Gain (2335ms)
      ✓ withdrawAssetGainToTrove(): A, B, C, D deposit -> 2 liquidations -> D withdraws -> 2 liquidations. Various deposit and liquidation vals. A, B, C, D withdraw correct KUSD deposit and ETH Gain (2145ms)
      ✓ withdrawAssetGainToTrove(): A, B, D deposit -> 2 liquidations -> C makes deposit -> 1 liquidation -> D withdraws -> 1 liquidation. All deposits: 100 KUSD. Liquidations: 100,100,100,50.  A, B, C, D withdraw correct KUSD deposit and ETH Gain (2543ms)
      ✓ withdrawAssetGainToTrove(): Depositor withdraws correct compounded deposit after liquidation empties the pool (1889ms)
      ✓ withdrawAssetGainToTrove(): Pool-emptying liquidation increases epoch by one, resets scaleFactor to 0, and resets P to 1e18 (1891ms)
      ✓ withdrawAssetGainToTrove(): Depositors withdraw correct compounded deposit after liquidation empties the pool (2324ms)
      ✓ withdrawAssetGainToTrove(): single deposit fully offset. After subsequent liquidations, depositor withdraws 0 deposit and *only* the ETH Gain from one liquidation (1719ms)
      ✓ withdrawAssetGainToTrove(): Depositor withdraws correct compounded deposit after liquidation empties the pool (3958ms)
      ✓ withdrawAssetGainToTrove(): deposit spans one scale factor change: Single depositor withdraws correct compounded deposit and ETH Gain after one liquidation (1276ms)
      ✓ withdrawAssetGainToTrove(): Several deposits of varying amounts span one scale factor change. Depositors withdraw correct compounded deposit and ETH Gain after one liquidation (4690ms)
      ✓ withdrawAssetGainToTrove(): deposit spans one scale factor change: Single depositor withdraws correct compounded deposit and ETH Gain after one liquidation (1460ms)
      ✓ withdrawAssetGainToTrove(): Several deposits of varying amounts span one scale factor change. Depositors withdraws correct compounded deposit and ETH Gain after one liquidation (1986ms)
alice deposit: 0
      ✓ withdrawAssetGainToTrove(): Deposit that decreases to less than 1e-9 of it's original value is reduced to 0 (3725ms)
      ✓ withdrawAssetGainToTrove(): Several deposits of 10000 KUSD span one scale factor change. Depositors withdraws correct compounded deposit and ETH Gain after one liquidation (2361ms)
      ✓ withdrawAssetGainToTrove(): 2 depositors can withdraw after each receiving half of a pool-emptying liquidation (3814ms)
      ✓ withdrawAssetGainToTrove(): Large liquidated coll/debt, deposits and ETH price (959ms)
      ✓ withdrawAssetGainToTrove(): Small liquidated coll/debt, large deposits and ETH price (784ms)

  Contract: StabilityPool
    Stability Pool Mechanisms
      ✓ provideToSP(): increases the Stability Pool KUSD balance (354ms)
      ✓ provideToSP(): updates the user's deposit record in StabilityPool (355ms)
      ✓ provideToSP(): reduces the user's KUSD balance by the correct amount (321ms)
      ✓ provideToSP(): increases totalKUSDDeposits by correct amount (283ms)
      ...
      ✓ provideToSP(): reverts if user tries to provide more than their KUSD balance (1030ms)
      ✓ provideToSP(): reverts if user tries to provide 2^256-1 KUSD, which exceeds their balance (508ms)
      ...
      ✓ provideToSP(): providing 0 KUSD reverts (1091ms)
      ✓ provideToSP(), new deposit: when SP > 0, triggers KUMO reward event - increases the sum G (1072ms)
      ...
      ✓ provideToSP(), new deposit: depositor does not receive any KUMO rewards (671ms)
      ✓ provideToSP(), new deposit after past full withdrawal: depositor does not receive any KUMO rewards (1679ms)
      ✓ provideToSP(), new eligible deposit: tagged front end receives KUMO rewards (1605ms)
      ...
      ✓ provideToSP(), topup: triggers KUMO reward event - increases the sum G (1007ms)
      ✓ provideToSP(), topup from different front end: doesn't change the front end tag (1511ms)
      ✓ provideToSP(), topup: depositor receives KUMO rewards (1487ms)
      ✓ provideToSP(), topup: tagged front end receives KUMO rewards (1036ms)
      ...
      ✓ withdrawFromSP(): partial retrieval - retrieves correct KUSD amount and the entire ETH Gain, and updates deposit (1102ms)
      ✓ withdrawFromSP(): partial retrieval - leaves the correct amount of KUSD in the Stability Pool (1128ms)
      ✓ withdrawFromSP(): full retrieval - leaves the correct amount of KUSD in the Stability Pool (1091ms)
      ...
      ✓ withdrawFromSP(): it correctly updates the user's KUSD and ETH snapshots of entitled reward per unit staked (1049ms)
      ...
      ✓ withdrawFromSP(): increases depositor's KUSD token balance by the expected amount (2098ms)
      ...
      ✓ withdrawFromSP(): withdrawing 0 KUSD doesn't alter the caller's deposit or the total KUSD in the Stability Pool (993ms)
      ...
      ✓ withdrawFromSP(): Request to withdraw 2^256-1 KUSD only withdraws the caller's compounded deposit (1229ms)
      ...
      ✓ withdrawFromSP(): triggers KUMO reward event - increases the sum G (982ms)
      ✓ withdrawFromSP(), partial withdrawal: doesn't change the front end tag (1421ms)
      ✓ withdrawFromSP(), partial withdrawal: depositor receives KUMO rewards (1155ms)
      ✓ withdrawFromSP(), partial withdrawal: tagged front end receives KUMO rewards (1245ms)
      ...
      ✓ withdrawAssetGainToTrove(): reverts when user has no active deposit (1043ms)
      ✓ withdrawAssetGainToTrove(): Applies kusdLoss to user's deposit, and redirects ETH reward to user's Trove (932ms)
      ✓ withdrawAssetGainToTrove(): reverts if it would leave trove with ICR < MCR (3920ms)
      ✓ withdrawAssetGainToTrove(): Subsequent deposit and withdrawal attempt from same account, with no intermediate liquidations, withdraws zero ETH (976ms)
      ✓ withdrawAssetGainToTrove(): decreases StabilityPool ETH and increases activePool ETH (976ms)
      ✓ withdrawAssetGainToTrove(): All depositors are able to withdraw their ETH gain from the SP to their Trove (5974ms)
      ✓ withdrawAssetGainToTrove(): All depositors withdraw, each withdraw their correct ETH gain (2591ms)
      ✓ withdrawAssetGainToTrove(): caller can withdraw full deposit and ETH gain to their trove during Recovery Mode (1629ms)
      ✓ withdrawAssetGainToTrove(): reverts if user has no trove (967ms)
      ✓ withdrawAssetGainToTrove(): triggers KUMO reward event - increases the sum G (1298ms)
      ✓ withdrawAssetGainToTrove(), partial withdrawal: doesn't change the front end tag (4778ms)
      ✓ withdrawAssetGainToTrove(), eligible deposit: depositor receives KUMO rewards (1607ms)
      ✓ withdrawAssetGainToTrove(), eligible deposit: tagged front end receives KUMO rewards (1701ms)
      ✓ withdrawAssetGainToTrove(), eligible deposit: tagged front end's stake decreases (5345ms)
      ✓ withdrawAssetGainToTrove(), eligible deposit: tagged front end's snapshots update (1984ms)
      ✓ withdrawAssetGainToTrove(): reverts when depositor has no ETH gain (1172ms)
...
B stake after A11: 39999999999999999998008332745
  ...
  Contract: TroveManager - in Recovery Mode
    ...
    ✓ liquidate(), with 110% < ICR < TCR, and StabilityPool KUSD > debt to liquidate: offsets the trove entirely with the pool (1265ms)
    ✓ liquidate(), with ICR% = 110 < TCR, and StabilityPool KUSD > debt to liquidate: offsets the trove entirely with the pool, there’s no collateral surplus (1382ms)
    ✓ liquidate(), with  110% < ICR < TCR, and StabilityPool KUSD > debt to liquidate: removes stake and updates totalStakes (1361ms)
    ✓ liquidate(), with  110% < ICR < TCR, and StabilityPool KUSD > debt to liquidate: updates system snapshots (1287ms)
    ✓ liquidate(), with 110% < ICR < TCR, and StabilityPool KUSD > debt to liquidate: closes the Trove (6532ms)
    ✓ liquidate(), with 110% < ICR < TCR, and StabilityPool KUSD > debt to liquidate: can liquidate troves out of order (2667ms)
    ✓ liquidate(), with ICR > 110%, and StabilityPool KUSD < liquidated debt: Trove remains active (985ms)
    ✓ liquidate(), with ICR > 110%, and StabilityPool KUSD < liquidated debt: Trove remains in TroveOwners array (1134ms)
    ✓ liquidate(), with ICR > 110%, and StabilityPool KUSD < liquidated debt: nothing happens (1146ms)
    ✓ liquidate(), with ICR > 110%, and StabilityPool KUSD < liquidated debt: updates system shapshots (1284ms)
    ✓ liquidate(), with ICR > 110%, and StabilityPool KUSD < liquidated debt: causes correct Pool offset and ETH gain, and doesn't redistribute to active troves (1266ms)
    ✓ liquidate(), with ICR > 110%, and StabilityPool KUSD < liquidated debt: ICR of non liquidated trove does not change (2161ms)
    ✓ liquidate() with ICR > 110%, and StabilityPool KUSD < liquidated debt: total liquidated coll and debt is correct (1912ms)
...
gasUsed:  636956
    ...
    ✓ batchLiquidateTroves(), with 110% < ICR < TCR, and StabilityPool KUSD > debt to liquidate: can liquidate troves out of order (1357ms)
  ...
  Contract: TroveManager
    ...
    ✓ liquidate(): decreases ActivePool ETH and KUSDDebt by correct amounts (567ms)
    ✓ liquidate(): increases DefaultPool ETH and KUSD debt by correct amounts (553ms)
    ...
    ✓ liquidate(): updates the L_ETH and L_KUSDDebt reward-per-unit-staked totals (984ms)
    ...
    ✓ liquidate(): when SP > 0, triggers KUMO reward event - increases the sum G (1081ms)
    ...
    ✓ liquidateTroves(): when SP > 0, triggers KUMO reward event - increases the sum G (5057ms)
    ...
    ✓ batchLiquidateTroves: when SP > 0, triggers KUMO reward event - increases the sum G (1428ms)
    ...
    ✓ redeemCollateral(): cancels the provided KUSD with debt from Troves with the lowest ICRs and sends an equivalent amount of Ether (838ms)
    ...
    ✓ redeemCollateral(): caller can redeem their entire KUSDToken balance (1086ms)
    ✓ redeemCollateral(): reverts when requested redemption amount exceeds caller's KUSD token balance (1320ms)
    ✓ redeemCollateral(): value of issued ETH == face value of redeemed KUSD (assuming 1 KUSD has value of $1) (1353ms)
    ...
    ✓ redeemCollateral(): a redemption made at zero base rate send a non-zero ETHFee to KUMO staking contract (5136ms)
    ✓ redeemCollateral(): a redemption made at zero base increases the ETH-fees-per-KUMO-staked in KUMO Staking contract (1207ms)
    ✓ redeemCollateral(): a redemption made at a non-zero base rate send a non-zero ETHFee to KUMO staking contract (1730ms)
    ✓ redeemCollateral(): a redemption made at a non-zero base rate increases ETH-per-KUMO-staked in the staking contract (5971ms)
    ...
    ✓ getPendingKUSDDebtReward(): Returns 0 if there is no pending KUSDDebt reward (618ms)
    ✓ getPendingETHReward(): Returns 0 if there is no pending ETH reward (620ms)
    ✓ computeICR(): Returns 0 if trove's coll is worth 0
    ✓ computeICR(): Returns 2^256-1 for ETH:USD = 100, coll = 1 ETH, debt = 100 KUSD
    ✓ computeICR(): returns correct ICR for ETH:USD = 100, coll = 200 ETH, debt = 30 KUSD
    ✓ computeICR(): returns correct ICR for ETH:USD = 250, coll = 1350 ETH, debt = 127 KUSD
    ✓ computeICR(): returns correct ICR for ETH:USD = 100, coll = 1 ETH, debt = 54321 KUSD
