...
contract SortedTroves is Ownable, CheckContract, ISortedTroves {
     ...
     */
    function _descendList(address _asset, ITroveManager _troveManager, uint256 _NICR, address _startId) internal view returns (address, address) {
        // If `_startId` is the head, check if the insert position is before the head
        if (data[_asset].head == _startId && _NICR >= _troveManager.getNominalICR(_asset, _startId)) {
        ...
        while (prevId != address(0) && !_validInsertPosition(_asset, _troveManager, _NICR, prevId, nextId)) {
     ...
     */
    function _ascendList(address _asset, ITroveManager _troveManager, uint256 _NICR, address _startId) internal view returns (address, address) {
        // If `_startId` is the tail, check if the insert position is after the tail
        if (data[_asset].tail == _startId && _NICR <= _troveManager.getNominalICR(_asset, _startId)) {
        ...
        while (nextId != address(0) && !_validInsertPosition(_asset, _troveManager, _NICR, prevId, nextId)) {
    ...
    function _findInsertPosition(address _asset, ITroveManager _troveManager, uint256 _NICR, address _prevId, address _nextId) internal view returns (address, address) {
        address prevId = _prevId;
        address nextId = _nextId;
