...
contract TroveManager is KumoBase, CheckContract, ITroveManager {
    ...
    */
    ...
    /*
    * L_ETH and L_KUSDDebt track the sums of accumulated liquidation rewards per unit staked. During its lifetime, each stake earns:
    *
    * An ETH gain of ( stake * [L_ETH - L_ETH(0)] )
    * A KUSDDebt increase  of ( stake * [L_KUSDDebt - L_KUSDDebt(0)] )
    *
    * Where L_ETH(0) and L_KUSDDebt(0) are snapshots of L_ETH and L_KUSDDebt for the active Trove taken at the instant the stake was made
    */
    uint public L_ETH;
    uint public L_KUSDDebt;

    // Map addresses with active troves to their RewardSnapshot
    mapping (address => RewardSnapshot) public rewardSnapshots;

    // Object containing the ETH and KUSD snapshots for a given active trove
    struct RewardSnapshot { uint ETH; uint KUSDDebt;}
    ...
    uint public lastKUSDDebtError_Redistribution;
    ...
    **/
    ...
    struct RedemptionTotals {
        uint remainingKUSD;
        uint totalKUSDToRedeem;
        uint totalETHDrawn;
        uint ETHFee;
        uint ETHToSendToRedeemer;
        uint decayedBaseRate;
        uint price;
        uint totalKUSDSupplyAtStart;
    }
    ...
    // event LTermsUpdated(uint _L_ETH, uint _L_KUSDDebt);
    // event TroveSnapshotsUpdated(uint _L_ETH, uint _L_KUSDDebt);
    ...
    */
    function liquidateTroves(uint _n) external override {
        ...
        if (vars.recoveryModeAtStart) {
            totals = _getTotalsFromLiquidateTrovesSequence_RecoveryMode(contractsCache, vars.price, vars.KUSDInStabPool, _n);
        } else { // if !vars.recoveryModeAtStart
            totals = _getTotalsFromLiquidateTrovesSequence_NormalMode(contractsCache.activePool, contractsCache.defaultPool, vars.price, vars.KUSDInStabPool, _n);
        }
    ...
    */
    function batchLiquidateTroves(address[] memory _troveArray) public override {
        ...
        if (vars.recoveryModeAtStart) {
            totals = _getTotalFromBatchLiquidate_RecoveryMode(activePoolCached, defaultPoolCached, vars.price, vars.KUSDInStabPool, _troveArray);
        } else {  //  if !vars.recoveryModeAtStart
            totals = _getTotalsFromBatchLiquidate_NormalMode(activePoolCached, defaultPoolCached, vars.price, vars.KUSDInStabPool, _troveArray);
        }
    ...
    */
    ...
    function _sendGasCompensation(IActivePool _activePool, address _liquidator, uint _KUSD, uint _ETH) internal {
    ...
    function _movePendingTroveRewardsToActivePool(IActivePool _activePool, IDefaultPool _defaultPool, uint _KUSD, uint _ETH) internal {
        _defaultPool.decreaseKUSDDebt(_KUSD);
        _activePool.increaseKUSDDebt(_KUSD);
    ...
    */
    function _redeemCloseTrove(ContractsCache memory _contractsCache, address _borrower, uint _KUSD, uint _ETH) internal {
        _contractsCache.kusdToken.burn(gasPoolAddress, _KUSD);
        // Update Active Pool KUSD, and send ETH to account
        _contractsCache.activePool.decreaseKUSDDebt(_KUSD);
    ...
    */
        ...
        override
    {
        ...
        totals.totalKUSDSupplyAtStart = getEntireSystemDebt();
        ...
        _updateBaseRateFromRedemption(totals.totalETHDrawn, totals.price, totals.totalKUSDSupplyAtStart);
        ...
        emit Redemption(_KUSDamount, totals.totalKUSDToRedeem, totals.totalETHDrawn, totals.ETHFee);
        ...
        // Update Active Pool KUSD, and send ETH to account
        contractsCache.activePool.decreaseKUSDDebt(totals.totalKUSDToRedeem);
    ...
    function _getCurrentTroveAmounts(address _borrower) internal view returns (uint, uint) {
        ...
        uint pendingKUSDDebtReward = getPendingKUSDDebtReward(_borrower);

        uint currentETH = Troves[_borrower].coll.add(pendingETHReward);
        uint currentKUSDDebt = Troves[_borrower].debt.add(pendingKUSDDebtReward);

        return (currentETH, currentKUSDDebt);
    }
    ...
    function _applyPendingRewards(IActivePool _activePool, IDefaultPool _defaultPool, address _borrower) internal {
        if (hasPendingRewards(_borrower)) {
            ...
            Troves[_borrower].debt = Troves[_borrower].debt.add(pendingKUSDDebtReward);

            _updateTroveRewardSnapshots(_borrower);

            // Transfer from DefaultPool to ActivePool
            _movePendingTroveRewardsToActivePool(_activePool, _defaultPool, pendingKUSDDebtReward, pendingETHReward);
    ...
    // Update borrower's snapshots of L_ETH and L_KUSDDebt to reflect the current values
    ...
    function _updateTroveRewardSnapshots(address _borrower) internal {
        ...
        rewardSnapshots[_borrower].KUSDDebt = L_KUSDDebt;
        emit TroveSnapshotsUpdated(L_ETH, L_KUSDDebt);
    }
    ...
    function getPendingKUSDDebtReward(address _borrower) public view override returns (uint) {
        uint snapshotKUSDDebt = rewardSnapshots[_borrower].KUSDDebt;
        uint rewardPerUnitStaked = L_KUSDDebt.sub(snapshotKUSDDebt);

        if ( rewardPerUnitStaked == 0 || Troves[_borrower].status != Status.active) { return 0; }

        uint stake =  Troves[_borrower].stake;

        uint pendingKUSDDebtReward = stake.mul(rewardPerUnitStaked).div(DECIMAL_PRECISION);
    ...
    function getEntireDebtAndColl(
        ...
        view
        override
        returns (uint debt, uint coll, uint pendingKUSDDebtReward, uint pendingETHReward)
    {
        debt = Troves[_borrower].debt;
        coll = Troves[_borrower].coll;

        pendingKUSDDebtReward = getPendingKUSDDebtReward(_borrower);
    ...
    function _redistributeDebtAndColl(IActivePool _activePool, IDefaultPool _defaultPool, uint _debt, uint _coll) internal {
        ...
        * error correction, to keep the cumulative error low in the running totals L_ETH and L_KUSDDebt:
        *
        ...
        */
        ...
        uint KUSDDebtNumerator = _debt.mul(DECIMAL_PRECISION).add(lastKUSDDebtError_Redistribution);

        // Get the per-unit-staked terms
        uint ETHRewardPerUnitStaked = ETHNumerator.div(totalStakes);
        uint KUSDDebtRewardPerUnitStaked = KUSDDebtNumerator.div(totalStakes);

        lastETHError_Redistribution = ETHNumerator.sub(ETHRewardPerUnitStaked.mul(totalStakes));
        lastKUSDDebtError_Redistribution = KUSDDebtNumerator.sub(KUSDDebtRewardPerUnitStaked.mul(totalStakes));

        // Add per-unit-staked terms to the running totals
        L_ETH = L_ETH.add(ETHRewardPerUnitStaked);
        L_KUSDDebt = L_KUSDDebt.add(KUSDDebtRewardPerUnitStaked);

        emit LTermsUpdated(L_ETH, L_KUSDDebt);

        // Transfer coll and debt from ActivePool to DefaultPool
        _activePool.decreaseKUSDDebt(_debt);
        _defaultPool.increaseKUSDDebt(_debt);
    ...
    function _closeTrove(address _borrower, Status closedStatus) internal {
        ...
        rewardSnapshots[_borrower].KUSDDebt = 0;
    ...
    */
    function _updateBaseRateFromRedemption(uint _ETHDrawn,  uint _price, uint _totalKUSDSupply) internal returns (uint) {
    ...
    function getBorrowingFee(uint _KUSDDebt) external view override returns (uint) {
        return _calcBorrowingFee(getBorrowingRate(), _KUSDDebt);
    }

    function getBorrowingFeeWithDecay(uint _KUSDDebt) external view override returns (uint) {
        return _calcBorrowingFee(getBorrowingRateWithDecay(), _KUSDDebt);
    }

    function _calcBorrowingFee(uint _borrowingRate, uint _KUSDDebt) internal pure returns (uint) {
    ...
    function _requireKUSDBalanceCoversRedemption(IKUSDToken _kusdToken, address _redeemer, uint _amount) internal view {
